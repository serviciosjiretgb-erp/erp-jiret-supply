...
export default function App() {
    ...
    categories.forEach(cat => {
       const itemsData = (inventory || []).filter(i => (i.category || 'Otros') === cat).map(item => {
          ...
          movs.filter(m => m.timestamp >= start && m.timestamp <= end).forEach(m => {
             ...
             if (isPos) { mEntQty += m.qty; mEntCost += (m.cost * m.qty); } 
             else { mSalQty += m.qty; mSalCost += (m.cost * m.qty); }
          });
  ...
  // ============================================================================
  // LOGICA VENTAS Y FACTURACIÓN (INTACTO)
  // ============================================================================
  ...
  const renderRecipeInventoryOptions = () => {
    ...
    return (<><option value="">Seleccione Insumo / Material...</option>{Object.keys(grouped).map(cat => (<optgroup key={cat} label={`📌 ${cat.toUpperCase()}`}>{(grouped[cat] || []).map(i => <option key={i.id} value={i.id}>{i.id} - {i.desc} ({i.stock} {i.unit})</option>)}</optgroup>))}</>);
  };
  ...
  const renderPhaseInventoryOptions = () => {
    const grouped = {}; (inventory || []).forEach(i => { const cat = i.category || 'Otros'; if (!grouped[cat]) grouped[cat] = []; grouped[cat].push(i); });
    return (<><option value="">Seleccione Insumo...</option>{Object.keys(grouped).map(cat => (<optgroup key={cat} label={`📌 ${cat.toUpperCase()} (Recomendado)`}>{(grouped[cat] || []).map(i => <option key={i.id} value={i.id}>{i.id} - {i.desc} ({i.stock} {i.unit})</option>)}</optgroup>))}</>);
  };
  ...
  const handleSavePhase = async (e) => {
    ...
    if (isSkip) { currentPhase.skipped = true; currentPhase.isClosed = true; } 
        ...
        if (prodKg > 0 || mermaKg > 0 || (phaseForm.insumos || []).length > 0) {
            ...
            await batch.commit();
            const newBatch = { id: Date.now().toString(), timestamp: Date.now(), date: phaseForm.date, insumos: phaseForm.insumos, producedKg: prodKg, mermaKg, totalInsumosKg, cost: phaseCost, operator: appUser?.name || 'Operador' };
  ...
  // NUEVO: Función para reabrir fases cerradas (REQUERIMIENTO DEL USUARIO)
  ...
  const renderProductionModule = () => {
    ...
    return (
      <div className="animate-in fade-in space-y-6">
        ...
        {/* CALCULADORA / SIMULADOR OP MODIFICADO CON COSTOS */}
    ...
    return (
      <div id="pdf-content" className="bg-white p-12 print:p-0 min-h-screen text-black shadow-none border-0 font-black uppercase"><style>{`@media print { @page { size: landscape; margin: 5mm; } body { background-color: white !important; } .text-orange-600 { color: #ea580c !important; } }`}</style>
        ...
        <div className="flex justify-between items-end border-b-2 border-black pb-4 mb-6">
           <div className="flex items-center gap-4">
              <div className="bg-black text-white p-2 rounded-lg flex items-center print:border print:border-black print:bg-white print:text-black font-black text-2xl">
                 G&B
              </div>
              <div>
        ...
        <div className="overflow-hidden rounded-xl border border-gray-300">
           <table className="w-full text-left text-[10px] whitespace-nowrap">
              <thead className="bg-gray-100 text-gray-800 border-b border-gray-300">
                 <tr>
                    <th className="p-3 font-black uppercase">FASE / CONCEPTO</th>
                    <th className="p-3 text-center font-black uppercase">CANTIDAD</th>
                    <th className="p-3 text-center font-black uppercase">U.M.</th>
                    <th className="p-3 font-black uppercase">NOTAS / INDICADORES</th>
                 </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                 ...
                 {mpConsumida.length > 0 ? mpConsumida.map((ing, i) => (
                   <tr key={i} className="hover:bg-gray-50">
                     <td className="p-2 pl-4 font-bold text-gray-800 font-black">{(inventory || []).find(inv=>inv.id===ing.id)?.desc || ing.id}</td>
                     <td className="p-2 text-center text-gray-700 font-black">{formatNum(ing.qty)}</td>
                     <td className="p-2 text-center text-gray-500 font-black">kg</td>
                     <td className="p-2 text-gray-600 font-black">{formatNum(((ing.qty||0)/totalMezclaPreparada)*100)}% de la mezcla</td>
                   </tr>
                 )) : (
                   <tr><td colSpan="4" className="p-4 text-center text-gray-500 italic">Sin reporte de insumos.</td></tr>
                 )}
                 <tr className="bg-gray-100 font-black border-y-2 border-gray-300 font-black">
                   <td className="p-2 pl-4 text-black font-black">TOTAL MATERIA PRIMA UTILIZADA</td>
                   <td className="p-2 text-center text-black font-black">{formatNum(totalMezclaPreparada)}</td>
                   <td className="p-2 text-center text-gray-600 font-black">kg</td>
                   <td className="p-2 text-gray-600 font-black">Ingreso real a Planta</td>
                 </tr>
                 ...
                 <tr className="bg-black font-black border-y-4 border-orange-500 text-[12px] text-white print:border-black print:bg-gray-200 print:text-black font-black">
                   <td className="p-3 pl-4 font-black">PRODUCCIÓN FINAL LÍQUIDA</td>
                   <td className="p-3 text-center text-orange-600 font-black">{formatNum(totalFinalUnidades)}</td>
                   <td className="p-3 text-center text-orange-600 font-black">{unitFinal}</td>
                   <td className="p-3 text-gray-400 print:text-gray-700 text-[9px] font-black">Unidades empacadas final</td>
                 </tr>
              </tbody>
           </table>
        </div>
