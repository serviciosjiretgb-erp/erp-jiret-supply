...
export default function App() {
  ...
  const clearAllReports = () => {
    ...
    setShowPhaseReport(null); setShowFiniquito(null);
    ...
    setDialog({ title: 'Anular Movimiento', text: `¿Desea revertir este movimiento?`, type: 'confirm', onConfirm: async () => {
        ...
        if (item) {
          ...
          setDialog({title: 'Anulado', text: 'Movimiento revertido y stock actualizado.', type: 'alert'});
           ...
           setDialog({title: 'Anulado', text: 'Registro eliminado del 
historial.', type: 'alert'});
        }
    }});
  };
  const generateReport177Data = () => {
    ...
    categories.forEach(cat => {
       const itemsData = (inventory || []).filter(i => (i.category || 'Otros') === cat).map(item => {
          ...
          let monthEntradasQty = 0, monthEntradasCost = 0, monthSalidasQty 
= 0, monthSalidasCost = 0;
          movs.filter(m => m.timestamp >= startDate && m.timestamp <= endDate).forEach(m => {
             const isPos = m.type === 'ENTRADA' || m.type === 'AJUSTE (POSITIVO)';
             if (isPos) { monthEntradasQty += m.qty;
monthEntradasCost += (m.cost * m.qty); } 
             else { monthSalidasQty += m.qty;
monthSalidasCost += (m.cost * m.qty); }
          });
          const invFinalQty = initialStock + monthEntradasQty - monthSalidasQty;
          return { ...item, initialStock, initialTotal: initialStock * item.cost, monthEntradasQty, monthEntradasTotal: monthEntradasCost, monthEntradasProm: monthEntradasQty > 0 ?
monthEntradasCost / monthEntradasQty : 0, monthSalidasQty, monthSalidasTotal: monthSalidasCost, monthSalidasProm: monthSalidasQty > 0 ?
monthSalidasCost / monthSalidasQty : 0, invFinalQty, invFinalTotal: invFinalQty * item.cost, invFinalCost: item.cost };
       });
  ...
  const handleInvoiceFormChange = (field, value) => {
    let f = { ...newInvoiceForm, [field]: typeof value === 'string' ?
value.toUpperCase() : value };
    ...
    if (field === 'opAsignada') {
       ...
       f.productoMaquilado = op ? `OP N°: ${String(op.id).replace('OP-', '').padStart(5, '0')} | PRODUCTO: ${op.tipoProducto} | ESPECIFICACIONES: ${op.desc} |
CANTIDAD: ${formatNum(op.cantidad)} ${op.presentacion}` : '';
    }
    let base = parseNum(field === 'montoBase' ? value : f.montoBase);
    let applyIva = (field === 'aplicaIva' ? value : f.aplicaIva) === 'SI';
    if (applyIva) { const ivaCalc = base * 0.16; f.iva = ivaCalc > 0 ? ivaCalc.toFixed(2) : '';
  ...
  const handleReqFormChange = (field, value) => {
    let f = { ...newReqForm, [field]: typeof value === 'string' ?
value.toUpperCase() : value };
    if (field === 'client') { const c = (clients || []).find(cl => cl.name === (value||'').toUpperCase());
...
Math.round(m * 1000) : m;
      if (tipo === 'BOLSAS' && l > 0) {
         ...
         f.pesoMillar = 'N/A';
         f.desc = `TERMOENCOGIBLE ${w}CM X ${micFmt}MIC | ${f.color || ''}`; f.requestedKg = c > 0 ? c.toFixed(2) : '0.00';
      } else { f.pesoMillar = '0.00'; f.requestedKg = '0.00'; }
    } else { f.pesoMillar = tipo === 'TERMOENCOGIBLE' ?
'N/A' : '0.00'; f.requestedKg = f.presentacion === 'KILOS' && c > 0 ? c.toFixed(2) : '0.00';
    }
    setNewReqForm(f);
  };
  ...
  const startEditReq = (r) => { setEditingReqId(r.id);
  setNewReqForm({ fecha: r.fecha || getTodayDate(), client: r.client || '', tipoProducto: r.tipoProducto || 'BOLSAS', desc: r.desc || '', ancho: r.ancho || '', fuelles: r.fuelles || '', largo: r.largo || '', micras: r.micras || '', pesoMillar: r.tipoProducto === 'TERMOENCOGIBLE' ? 'N/A' : (r.pesoMillar || ''), presentacion: r.presentacion || 'MILLAR', cantidad: r.cantidad || '', requestedKg: r.requestedKg || '', color: r.color || 'NATURAL', tratamiento: r.tratamiento || 'LISO', vendedor: r.vendedor || '' });
  setShowNewReqPanel(true); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  ...
  // ============================================================================
  // LOGICA PRODUCCIÓN Y CONTROL DE FASES (ACTUALIZADO)
  // ============================================================================
  ...
  const handleSavePhase = async (e) => {
    ...
    if (isSkip) { currentPhase.skipped = true; currentPhase.isClosed = true; } 
        ...
        if (prodKg > 0 || mermaKg > 0 || (phaseForm.insumos || []).length > 0) {
            ...
            // Construimos el objeto de parámetros técnicos según la fase
            let techParams = {};
              ...
              return (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in slide-in-from-bottom-8">
                  ...
                  setPhaseForm(initialPhaseForm);}} className="bg-gray-100 p-2.5 rounded-xl text-gray-500 hover:text-black"><X size={18}/></button></div>
                    ...
                    {cPhase.isClosed ?
                    (<div className="text-center py-10 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200 shadow-inner">
                        <CheckCircle size={56} className="text-green-500 mx-auto mb-6"/>
                        <h4 className="text-xl font-black text-black uppercase tracking-widest">Esta Fase se encuentra cerrada</h4>
                        <p className="text-[10px] font-bold text-gray-400 uppercase mt-2 mb-6">Ya no se permiten reportes parciales en esta etapa.</p>
                        <button onClick={() => {
                            setDialog({ title: `REABRIR FASE`, text: `¿Seguro que desea reabrir esta fase para editar o añadir más lotes?`, type: 'confirm', onConfirm: async () => {
                                ...
                                await updateDoc(getDocRef('requirements', req.id), { 
                                    production: { ...req.production, [activePhaseTab]: currentPhase },
                                    status: newStatus
                                });
                                setDialog(null);
                            }});
                        }} className="bg-orange-500 text-white px-6 py-2 rounded-xl text-[10px] font-black uppercase shadow-md hover:bg-orange-600 transition-all flex items-center gap-2 mx-auto"><Edit size={14}/> REABRIR FASE PARA EDITAR</button>
                     </div>) : (
