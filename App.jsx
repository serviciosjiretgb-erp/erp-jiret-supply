...
export default function App() {
  ...
  // MÓDULO VENTAS (INTACTO SEGÚN TU PEDIDO)
  ...
  const handleSaveInvItem = async (e) => {
    ...
    try { await setDoc(getDocRef('inventory', itemData.id), itemData, { merge: true }); setNewInvItemForm(initialInvItemForm); } catch(err) { setDialog({ title: 'Error', text: err.message, type: 'alert' }); }
  };
  ...
  const handleSavePhase = async (e) => {
    ...
    if (prodKg > 0 || mermaKg > 0 || (phaseForm?.insumos || []).length > 0) {
        ...
        await batch.commit();
        const newBatch = { id: Date.now().toString(), timestamp: Date.now(), date: phaseForm?.date || getTodayDate(), insumos: phaseForm?.insumos || [], producedKg: prodKg, mermaKg, totalInsumosKg, operator: appUser?.name || 'Operador' };
      ...
      <div className="animate-in fade-in space-y-6">
        {invView === 'catalogo' && (
          <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div className="p-8"><div className="overflow-x-auto"><table className="w-full text-left whitespace-nowrap"><thead className="bg-gray-100 border-b-2 border-gray-200 font-black text-[10px] uppercase"><tr><th>Código</th><th>Descripción</th><th className="text-right">Stock</th></tr></thead><tbody className="divide-y divide-gray-100">{filteredInventory.map(inv => (<tr key={inv?.id} className="hover:bg-gray-50"><td className="py-4 px-4 font-black text-orange-600 text-xs">{inv?.id}</td><td className="py-4 px-4 font-black uppercase text-xs">{inv?.desc}</td><td className="py-4 px-4 text-right font-black text-blue-600 text-lg">{formatNum(inv?.stock)} <span className="text-xs text-gray-400">{inv?.unit}</span></td></tr>))}</tbody></table></div></div>
          </div>
        )}
      </div>
    );
                ...
                <div className="p-8 bg-gray-50/50 border-b">
                  <form onSubmit={handleCreateRequirement} className="bg-white p-10 rounded-3xl border border-gray-100 shadow-sm space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div><label className="text-[10px] font-bold text-gray-500 uppercase block mb-2">Producto</label>
        ...
        {prodView === 'calculadora' && (
          <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden animate-in fade-in print:border-none print:shadow-none print:m-0 print:p-0 print:block print:w-full">
            <div data-html2canvas-ignore="true" className="px-8 py-6 border-b border-gray-200 bg-gray-50 flex justify-between items-center print:hidden">
               <h2 className="text-xl font-black text-black uppercase flex items-center gap-3 tracking-tighter"><Calculator className="text-orange-500" size={24}/> Simulador</h2>
               <button onClick={() => handleExportPDF('Simulador_Produccion', true)} className="bg-black text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase shadow-md hover:bg-gray-800 transition-colors flex items-center gap-2"><Printer size={16}/> PDF</button>
            </div>
            ...
            <div id="pdf-content" className="grid grid-cols-1 lg:grid-cols-12 gap-0 print:block print:w-full">
               ...
               <div data-html2canvas-ignore="true" className="lg:col-span-4 border-r border-gray-200 bg-gray-50 p-8 print:hidden space-y-8">
                 <div>
                     <h3 className="text-xs font-black uppercase text-black mb-4 border-b border-gray-200 pb-2">1. Mezcla</h3>
               ...
               <div className="lg:col-span-8 p-10 bg-white print:w-full print:p-0">
                  <div className="hidden print:block mb-6"><ReportHeader /><h1 className="text-xl font-black text-black uppercase border-b-2 border-orange-500 pb-1 mt-2">PROYECCIÓN DE PRODUCCIÓN</h1></div>
                  <div className="overflow-x-auto rounded-xl border border-gray-300 print:border-black print:rounded-none">
                     <table className="w-full text-left text-[10px] whitespace-nowrap print-text-xs text-black">
                        <thead className="bg-gray-200 print:bg-gray-300 border-b border-gray-400 print:border-black font-black uppercase"><tr><th className="p-2">Fase / Concepto</th><th className="p-2 text-center">Cantidad</th><th className="p-2 text-center">U.M.</th><th className="p-2">Notas</th></tr></thead>
                        <tbody className="text-black divide-y divide-gray-200 print:divide-black">
                           <tr className="bg-gray-100 font-black"><td>MEZCLA PROCESADA</td><td className="text-center">{formatNum(calcTotalMezcla)}</td><td className="text-center">kg</td><td>Entrada Planta</td></tr>
                           <tr className="bg-green-50 font-black text-green-800"><td>PRODUCCIÓN ESTIMADA</td><td className="text-center">{formatNum(calcProduccionFinalUnidades)}</td><td className="text-center">{simUmFinal}</td><td>Rendimiento estimado</td></tr>
                        </tbody>
                     </table>
                  </div>
               </div>
  ...
  const renderWorkOrder = () => {
    ...
    return (
      <div id="pdf-content" className="bg-white p-6 print:p-0 min-h-screen text-black shadow-none border-0"><style>{`@media print { @page { size: portrait; margin: 5mm; } }`}</style>
        ...
        <div className="mt-8 grid grid-cols-2 text-center font-black text-[9px] uppercase border-t-2 border-black pt-2 text-black font-black"><div>CONTROL DE CALIDAD</div><div>SUPERVISOR DE PLANTA</div></div>
      </div>
    );
  };
