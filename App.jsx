...
const calcRequestedKg = (tipoProducto, presentacion, cantidad, pesoMillar) => {
  // ✅ CORREGIDO: Para TERMOENCOGIBLE, los KG los proporciona el cliente directamente
  if (tipoProducto === 'TERMOENCOGIBLE') return cantidad;
  return presentacion === 'KILOS' ? cantidad : pesoMillar * cantidad;
};

const buildProductDescription = (tipoProducto, ancho, fuelles, largo, micras, color, categoria) => {
  const micFmt = micras < 1 && micras > 0 ? Math.round(micras * 1000) : micras;
  const catStr = categoria ? ` | ${categoria}` : '';
  
  if (tipoProducto === 'BOLSAS') {
    return fuelles > 0 
      ? `(${ancho}+${fuelles/2}+${fuelles/2})X${largo}X${micFmt}MIC | ${color}${catStr}`
      : `${ancho}X${largo}X${micFmt}MIC | ${color}${catStr}`;
  }
  // ✅ CORREGIDO: TERMOENCOGIBLE = ANCHO * MICRAS (fórmula simplificada)
  return `TERMOENCOGIBLE ${ancho}CM X ${micFmt}MIC | ${color}${catStr}`;
};
              ...
              <form onSubmit={handleSaveRequirement} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input type="text" value={newRequirementForm.categoria || ''} onChange={e => setNewRequirementForm({ ...newRequirementForm, categoria: e.target.value.toUpperCase() })} placeholder="EJ: PAÑAL, BOLSA BASURA, ETC" className="w-full p-3 border-2 border-gray-200 rounded-xl outline-none focus:border-orange-500 font-bold text-sm uppercase" />
                  </div>
                </div>
  ...
  const renderProductionModule = () => {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="bg-white rounded-3xl shadow-xl border-2 border-gray-200 p-12 max-w-2xl text-center">
          <Factory size={80} className="text-gray-300 mx-auto mb-6" />
          <h2 className="text-2xl font-black text-black uppercase mb-4">Módulo de Producción</h2>
          <p className="text-gray-600 font-bold text-lg mb-6">
            Las funciones de gestión de producción se encuentran ahora en el módulo de <span className="text-orange-500">VENTAS</span>
          </p>
          <div className="bg-orange-50 border-2 border-orange-200 rounded-2xl p-6">
            <p className="text-sm font-black text-orange-700 uppercase mb-3">Para gestionar producción:</p>
            <ul className="text-left text-sm text-gray-700 font-bold space-y-2">
              <li>→ Ir a <span className="text-orange-600 font-black">VENTAS</span></li>
              <li>→ Seleccionar <span className="text-orange-600 font-black">OP / REQUISICIONES</span></li>
              <li>→ Crear y gestionar órdenes de producción</li>
            </ul>
          </div>
        </div>
      </div>
            ...
            <nav className="md:w-64 flex-shrink-0 space-y-4 print:hidden animate-in slide-in-from-left">
              ...
              {appUser?.permissions?.produccion && activeTab === 'produccion' && (
                <div className="bg-white rounded-3xl p-5 border border-gray-200 shadow-sm space-y-2">
                  ...
                  <p className="text-xs text-gray-500 italic p-4">Las funciones de producción se gestionan desde el módulo de Ventas (OP/Requisiciones)</p>
                </div>
              )}
              ...
              {appUser?.permissions?.costos && activeTab === 'costos' && (
                <div className="bg-white rounded-3xl p-5 border border-gray-200 shadow-sm space-y-2">
                  ...
                  <button onClick={() => { clearAllReports(); setCostosView('operativos'); }} className={`w-full flex items-center justify-start gap-3 px-5 py-4 text-[11px] font-black rounded-2xl transition-all ${costosView === 'operativos' ? 'bg-black text-white shadow-xl' : 'text-slate-500 hover:bg-slate-100'} uppercase`}><Wrench size={16} /> Registrar Costos</button>
                  <button onClick={() => { clearAllReports(); setCostosView('reportes'); }} className={`w-full flex items-center justify-start gap-3 px-5 py-4 text-[11px] font-black rounded-2xl transition-all ${costosView === 'reportes' ? 'bg-black text-white shadow-xl' : 'text-slate-500 hover:bg-slate-100'} uppercase`}><BarChart3 size={16} /> Reportes / Dashboard</button>
                </div>
              )}
