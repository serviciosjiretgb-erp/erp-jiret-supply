...
export default function App() {
  ...
  // --- ESTADO CALCULADORA (ACTUALIZADO) ---
  ...
  const handleSaveInvItem = async (e) => {
    e.preventDefault();
    if (!newInvItemForm.id || !newInvItemForm.desc) return setDialog({ title: 'Aviso', text: 'Código y descripción son obligatorios.', type: 'alert' });
    const itemData = {
      ...newInvItemForm,
  ...
  const handleSaveMovement = async (e) => {
    e.preventDefault();
    if (!newMovementForm.itemId || !newMovementForm.qty) return setDialog({title: 'Aviso', text: 'Seleccione un ítem y defina la cantidad.', type: 'alert'});
    ...
    if (!isAddition && item.stock < qty) return setDialog({title: 'Stock Insuficiente', text: `El inventario actual (${item.stock} ${item.unit}) no cubre esta salida.`, type: 'alert'});
    ...
    try {
      ...
      batch.set(getDocRef('inventoryMovements', movId), { id: movId, date: newMovementForm.date, itemId: item.id, itemName: item.desc, type: type, qty: qty, cost: movCost, totalValue: qty * movCost, reference: newMovementForm.reference.toUpperCase(), notes: newMovementForm.notes.toUpperCase(), timestamp: Date.now(), user: appUser?.name || 'Sistema' });
      batch.update(getDocRef('inventory', item.id), { stock: item.stock + stockChange, cost: isAddition && movCost > 0 ? movCost : item.cost });
      await batch.commit();
      setNewMovementForm(initialMovementForm);
      setDialog({title: 'Éxito', text: 'Movimiento de inventario registrado.', type: 'alert'});
    } catch (err) { setDialog({title: 'Error', text: err.message, type: 'alert'}); }
  };
  ...
  const handleDeleteMovement = (m) => {
    setDialog({ title: 'Anular Movimiento', text: `¿Desea revertir este movimiento?`, type: 'confirm', onConfirm: async () => {
  ...
  const generateReport177Data = () => {
    ...
    categories.forEach(cat => {
       const itemsData = (inventory || []).filter(i => (i?.category || 'Otros') === cat).map(item => {
          ...
          movs.filter(m => m.timestamp >= startDate).forEach(m => {
             const isPos = m.type === 'ENTRADA' || m.type === 'AJUSTE (POSITIVO)';
             if (isPos) initialStock -= m.qty; else initialStock += m.qty;
          });
          
          let monthEntradasQty = 0, monthEntradasCost = 0, monthSalidasQty = 0, monthSalidasCost = 0;
          movs.filter(m => m.timestamp >= startDate && m.timestamp <= endDate).forEach(m => {
             const isPos = m.type === 'ENTRADA' || m.type === 'AJUSTE (POSITIVO)';
             if (isPos) { monthEntradasQty += m.qty; monthEntradasCost += (m.cost * m.qty); } 
             else { monthSalidasQty += m.qty; monthSalidasCost += (m.cost * m.qty); }
          });
          
          const invFinalQty = initialStock + monthEntradasQty - monthSalidasQty;
          return { ...item, initialStock, initialTotal: initialStock * item.cost, monthEntradasQty, monthEntradasTotal: monthEntradasCost, monthEntradasProm: monthEntradasQty > 0 ? monthEntradasCost / monthEntradasQty : 0, monthSalidasQty, monthSalidasTotal: monthSalidasCost, monthSalidasProm: monthSalidasQty > 0 ? monthSalidasCost / monthSalidasQty : 0, invFinalQty, invFinalTotal: invFinalQty * item.cost, invFinalCost: item.cost };
       });
  ...
  const handleAddClient = async (e) => {
    ...
    if (!newClientForm.rif || !newClientForm.razonSocial) return setDialog({ title: 'Aviso', text: 'El RIF y la Razón Social son obligatorios.', type: 'alert' });
    const rifUpper = newClientForm.rif.toUpperCase().trim();
    try {
      await setDoc(getDocRef('clientes', rifUpper), { ...newClientForm, name: newClientForm.razonSocial.toUpperCase().trim(), rif: rifUpper, timestamp: Date.now() }, { merge: true });
      setNewClientForm(initialClientForm); setEditingClientId(null); setDialog({ title: '¡Éxito!', text: 'Cliente guardado correctamente en la nube.', type: 'alert' });
    } catch(err) { setDialog({ title: 'Error', text: err.message, type: 'alert' }); }
  };
  ...
  // ============================================================================
  // LOGICA VENTAS: FACTURACIÓN Y REQUISICIONES (SELLADO ORIGINAL)
  // ============================================================================
  const generateInvoiceId = () => `FAC-${((invoices || []).reduce((m, r) => Math.max(m, parseInt(String(r.id).replace(/\D/g, ''), 10) || 0), 0) + 1).toString().padStart(4, '0')}`;
  ...
  const handleInvoiceFormChange = (field, value) => {
    ...
    if (field === 'clientRif') {
       ...
       f.clientName = c?.name || ''; f.vendedor = (c?.vendedor || '').toUpperCase(); f.opAsignada = ''; f.productoMaquilado = '';
    }
    if (field === 'opAsignada') {
       const op = (requirements || []).find(r => r.id === value);
       f.productoMaquilado = op ? `OP N°: ${String(op.id).replace('OP-', '').padStart(5, '0')} | PRODUCTO: ${op.tipoProducto} | ESPECIFICACIONES: ${op.desc} | CANTIDAD: ${formatNum(op.cantidad)} ${op.presentacion}` : '';
    }
    let base = parseNum(field === 'montoBase' ? value : f.montoBase);
    let applyIva = (field === 'aplicaIva' ? value : f.aplicaIva) === 'SI';
  ...
  const handleCreateInvoice = async (e) => {
    ...
    try {
      await setDoc(getDocRef('maquilaInvoices', finalInvId), { ...newInvoiceForm, id: finalInvId, documento: finalInvId, montoBase: parseNum(newInvoiceForm.montoBase), iva: parseNum(newInvoiceForm.iva), total: parseNum(newInvoiceForm.total), timestamp: Date.now(), user: appUser?.name || 'Vendedor' });
  ...
  const generateReqId = () => `OP-${((requirements || []).reduce((m, r) => Math.max(m, parseInt(String(r.id).replace(/\D/g, ''), 10) || 0), 0) + 1).toString().padStart(5, '0')}`;
  ...
  const handleCreateRequirement = async (e) => {
    ...
    try {
      await setDoc(getDocRef('requirements', opId), { ...newReqForm, id: opId, timestamp: editingReqId ? (requirements || []).find(r => r?.id === editingReqId)?.timestamp : Date.now(), status: editingReqId ? (requirements || []).find(r => r?.id === editingReqId)?.status : 'PENDIENTE DE INGENIERÍA', viewedByPlanta: false }, { merge: true });
      setShowNewReqPanel(false); setNewReqForm(initialReqForm); setEditingReqId(null); setDialog({title: 'Éxito', text: `OP guardada.`, type: 'alert'});
    } catch(err) { setDialog({title: 'Error', text: err.message, type: 'alert'}); }
  };
    ...
    return (<>
      ...
      {Object.keys(grouped).map(cat => (
        <optgroup key={cat} label={`📌 ${cat.toUpperCase()}`}>
          {(grouped[cat] || []).map(i => <option key={i?.id} value={i?.id}>{i?.id} - {i?.desc}</option>)}
        </optgroup>
      ))}
    </>);
  };
  ...
  const handleSaveRecipe = async () => {
    ...
    (tempRecipe || []).forEach(ing => { const item = (inventory || []).find(i => i?.id === ing?.id); if(item) totalCost += ((item?.cost || 0) * (ing?.totalQty || 0)); });
    ...
    setDialog({ title: 'Éxito', text: 'Fórmula asignada.', type: 'alert' });
  };
  ...
  const handleRemovePhaseIng = (idx) => {
    setPhaseForm({...phaseForm, insumos: (phaseForm?.insumos || []).filter((_, i) => i !== idx)});
  };
  ...
  const handleSavePhase = async (e) => {
    ...
    const isSkip = actionType === 'skip'; const isClose = actionType === 'close';
    ...
    if (isSkip) { currentPhase.skipped = true; currentPhase.isClosed = true;
        ...
        if (prodKg > 0 || mermaKg > 0 || (phaseForm?.insumos || []).length > 0) {
            ...
            await batch.commit();
            // Recopilar Parámetros Técnicos Digitalmente (Sin crashear)
            let techParams = {};
            ...
            const newBatch = { id: Date.now().toString(), timestamp: Date.now(), date: phaseForm?.date || getTodayDate(), insumos: phaseForm?.insumos ||
            [], producedKg: prodKg, mermaKg, totalInsumosKg, cost: phaseCost, operator: appUser?.name || 'Operador', techParams };
  ...
  const handleDeleteBatch = async (reqId, phase, batchId) => {
    setDialog({ title: `ELIMINAR LOTE`, text: `¿Seguro que desea eliminar este lote parcial?`, type: 'confirm', onConfirm: async () => {
        ...
        if (bIdx >= 0) { 
          ...
          for (let ing of (batch?.insumos || [])) { const item = (inventory || []).find(i => i?.id === ing?.id); if (item) fbBatch.update(getDocRef('inventory', item.id), { stock: (item?.stock || 0) + (ing?.qty || 0) }); } 
  ...
  const handleEditBatch = (reqId, phase, batchId) => {
    setDialog({ title: `MODIFICAR LOTE`, text: `El lote volverá al formulario para su edición y el inventario se ajustará. ¿Continuar?`, type: 'confirm', onConfirm: async () => {
        const req = (requirements || []).find(r => r?.id === reqId); if(!req) return;
        let currentPhase = { ...(req?.production?.[phase] || {}) }; 
        const bIdx = (currentPhase?.batches || []).findIndex(b => b?.id === batchId);
        if (bIdx >= 0) 
        { 
            ...
            // Restaurar parámetros técnicos al formulario sin causar error
            if(phase === 'extrusion' && batch?.techParams) {
                restoreForm.operadorExt = batch.techParams.operador || ''; restoreForm.tratado = batch.techParams.tratado ||
                ...
                ''; restoreForm.zona2 = batch.techParams.zonas?.[1] || ''; restoreForm.zona3 = batch.techParams.zonas?.[2] || '';
                restoreForm.zona4 = batch.techParams.zonas?.[3] || ''; restoreForm.zona5 = batch.techParams.zonas?.[4] ||
            ...
            if(phase === 'impresion' && batch?.techParams) {
                ...
                ''; restoreForm.kgRecibidosImp = batch.techParams.kgRecibidos || ''; restoreForm.cantColores = batch.techParams.cantColores || '';
                restoreForm.relacionImp = batch.techParams.relacion || ''; restoreForm.motorImp = batch.techParams.motor ||
            ...
            if(phase === 'sellado' && batch?.techParams) {
                ...
                ''; restoreForm.kgRecibidosSel = batch.techParams.kgRecibidos || ''; restoreForm.impresa = batch.techParams.impresa || 'NO';
                restoreForm.tipoSello = batch.techParams.tipoSello || 'Sello FC';
                restoreForm.tempCabezalA = batch.techParams.tempCabezalA || ''; restoreForm.tempCabezalB = batch.techParams.tempCabezalB || '';
                restoreForm.tempPisoA = batch.techParams.tempPisoA || ''; restoreForm.tempPisoB = batch.techParams.tempPisoB || '';
            ...
            for (let ing of (batch?.insumos || [])) { const item = (inventory || []).find(i => i?.id === ing?.id);
  ...
  // NUEVA FUNCIÓN PARA REABRIR FASES CERRADAS
  const handleReopenPhase = async (reqId, phase) => {
    setDialog({ title: `REABRIR FASE`, text: `¿Desea reabrir esta fase para añadir o modificar lotes?`, type: 'confirm', onConfirm: async () => {
  ...
  // --- LÓGICA CALCULADORA (SIMULADOR OP MODIFICADO CON COSTOS) ---
  ...
  const calcCostoFinalUnidad = calcProduccionFinalUnidades > 0 ? (calcCostoMezclaPreparada / calcProduccionFinalUnidades) : 0;
    ...
    <div className="flex items-start justify-between border-b-2 border-black pb-4 mb-6 print:border-black print:w-full print:flex-row">
       <div className="flex flex-col items-start w-1/2 print:w-1/2">
          ...
          <div className="flex items-center -mt-2">
             <span className="text-black font-black text-[50px] leading-none">G</span><div className="bg-orange-500 text-white rounded-full w-8 h-8 flex items-center justify-center text-xl font-black mx-1 print:bg-orange-500 
print:text-black">&amp;</div><span className="text-black font-black text-[50px] leading-none">B</span>
          </div>
       </div>
       <div className="w-1/2 text-right print:w-1/2">
           ...
           <p className="text-[9px] font-medium text-gray-500 mt-1 uppercase print:text-black">Av.
Circunvalación Nro. 02 C.C. El Dividivi Local G-9, Maracaibo.</p>
       </div>
    </div>
  );
  const renderLogin = () => (
...
from-white/10 to-transparent transform -skew-x-12 pointer-events-none"></div>
    ...
    <form onSubmit={handleLogin} className="space-y-6">
                ...
                <div>
                  ...
                  <div className="relative group"><Lock className="absolute left-4 top-3.5 text-gray-400 group-hover:text-orange-500 transition-colors z-10" size={18}/><input type="password" value={loginData.password} onChange={e=>setLoginData({...loginData, password: e.target.value})} className="w-full pl-12 pr-4 py-3 border-2 border-gray-200 bg-gray-50 focus:bg-white focus:border-orange-500 rounded-xl 
text-sm font-black outline-none transition-all shadow-[inset_0_2px_6px_rgba(0,0,0,0.06)] hover:shadow-[inset_0_2px_8px_rgba(0,0,0,0.1)]" placeholder="••••••••"/></div>
                </div>
  ...
  const renderHome = () => (
    <div className="w-full max-w-6xl mx-auto py-8 animate-in fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 px-4">
        <button onClick={() => { clearAllReports(); setActiveTab('ventas'); setVentasView('facturacion'); }} className="group bg-black border-l-4 border-orange-500 rounded-3xl p-10 text-left hover:bg-gray-900 transition-all shadow-xl"><Users size={40} className="text-orange-500 mb-4" /><h3 className="text-xl 
font-black text-white uppercase">Ventas y Facturación</h3><p className="text-xs text-gray-400 mt-2">Directorio, OP y Facturación.</p></button>
    ...
    if (showGeneralInvoicesReport) {
      ...
      return (
        ...
        <thead className="bg-gray-100 uppercase"><tr><th className="p-2 border">Fecha</th><th className="p-2 border">Factura</th><th className="p-2 border">Cliente</th><th className="p-2 border text-right">Base ($)</th><th className="p-2 border text-right">IVA ($)</th><th className="p-2 border text-right">Total ($)</th></tr></thead>
            <tbody>{(invoices ||
[]).map(i => (<tr key={i?.id}><td className="p-2 border">{i?.fecha}</td><td className="p-2 border font-bold">{i?.documento}</td><td className="p-2 border">{i?.clientName}</td><td className="p-2 border text-right">${formatNum(i?.montoBase)}</td><td className="p-2 border text-right">${formatNum(i?.iva)}</td><td className="p-2 border text-right font-black text-green-600">${formatNum(i?.total)}</td></tr>))}</tbody>
    ...
    if (showSingleInvoice) {
      ...
      return (
        <div id="pdf-content" className="bg-white p-12 min-h-screen text-black"><div data-html2canvas-ignore="true" className="flex justify-between mb-8 print:hidden"><button onClick={() => setShowSingleInvoice(null)} className="bg-gray-100 px-6 py-2 rounded-xl font-black text-xs uppercase">Volver</button><button onClick={() => handleExportPDF(`Factura_${inv.documento}`)} className="bg-black text-white px-8 py-3 rounded-xl flex items-center gap-2 font-black text-xs uppercase shadow-lg"><Printer size={16} /> Exportar PDF</button></div><ReportHeader />
          ...
          <div className="grid grid-cols-2 gap-4 mb-8 text-sm uppercase font-bold"><div><p>CLIENTE: {inv.clientName}</p><p>RIF: {inv.clientRif}</p><p className="text-[10px] text-gray-500">DIRECCIÓN: {client.direccion || 'N/A'}</p></div><div className="text-right"><p>FECHA: {inv.fecha}</p><p>VENDEDOR: {inv.vendedor ||
'N/A'}</p></div></div>
          <table className="w-full border-collapse border-2 border-black mb-8"><thead className="bg-gray-100"><tr><th className="p-4 border-b border-black">Descripción Maquila</th><th className="p-4 border-b border-black text-center">Importe Base (USD)</th></tr></thead><tbody><tr><td className="p-4 border-r border-black font-bold text-sm">MAQUILA / PRODUCTO: {inv.productoMaquilado ||
'N/A'}</td><td className="p-4 text-center font-bold text-lg">${formatNum(inv.montoBase)}</td></tr></tbody></table>
    ...
    if (showSingleReqReport) {
      ...
      return (
        <div id="pdf-content" className="bg-white p-12 min-h-screen text-black shadow-xl"><div data-html2canvas-ignore="true" className="flex justify-between mb-8 print:hidden"><button onClick={() => setShowSingleReqReport(null)} className="bg-gray-100 px-6 py-2 rounded-xl font-black text-xs uppercase">Volver</button><button onClick={() => handleExportPDF(`Requisicion_${req.id}`)} className="bg-black text-white px-8 py-3 rounded-xl flex items-center gap-2 font-black text-xs uppercase shadow-lg"><Printer size={16} /> Exportar PDF</button></div><ReportHeader />
          ...
          <div className="grid grid-cols-2 gap-4 mb-6 font-bold text-sm uppercase"><div><p>CLIENTE: {req.client}</p><p className="mt-1">VENDEDOR: {req.vendedor || 
'N/A'}</p></div><div className="text-right"><p>FECHA: {req.fecha}</p><p className="mt-1">TIPO: {req.tipoProducto}</p></div></div>
          <div className="border-4 border-black p-6 grid grid-cols-4 gap-4 text-center text-[10px] font-black uppercase mb-6 rounded-3xl"><div>ANCHO<br/><span className="text-lg">{req.ancho} CM</span></div><div>FUELLES<br/><span className="text-lg">{req.fuelles ||
'0'} CM</span></div><div>LARGO<br/><span className="text-lg">{req.largo} CM</span></div><div>MICRAS<br/><span className="text-lg">{req.micras}</span></div></div>
          <div className="bg-gray-50 p-6 flex justify-between border-2 border-black rounded-3xl"><div><span className="block text-[10px] font-black uppercase">Peso Millar Estimado</span><span className="text-xl font-black">{req.pesoMillar ||
'N/A'}</span></div><div className="text-right"><span className="block text-[10px] font-black uppercase">Carga Total Planta</span><span className="text-3xl font-black text-orange-600">{formatNum(req.requestedKg)} KG</span></div></div>
      ...
      return (
        <div id="pdf-content" className="bg-white p-10 min-h-screen print:p-0 text-black">
          ...
          <table className="w-full text-[10px] border-collapse border border-gray-300">
            ...
            <tbody>{(clients ||
[]).map(c => (<tr key={c?.rif}><td className="p-2 border font-bold">{c?.rif}</td><td className="p-2 border font-black uppercase">{c?.name}</td><td className="p-2 border uppercase">{c?.direccion}</td><td className="p-2 border">{c?.telefono}</td><td className="p-2 border uppercase font-bold">{c?.vendedor}</td></tr>))}</tbody>
          </table>
        </div>
      );
    }
...
[]).map(r => (<tr key={r?.id}><td className="p-2 border text-center">{String(r?.id).replace('OP-', '').padStart(5, '0')}</td><td className="p-2 border">{r?.fecha}</td><td className="p-2 border font-bold">{r?.client}</td><td className="p-2 border">{r?.vendedor}</td><td className="p-2 border">{r?.desc}</td><td className="p-2 border text-right font-black">{formatNum(r?.requestedKg)} KG</td><td className="p-2 border text-center font-bold uppercase">{r?.status}</td></tr>))}</tbody>
    ...
    return (
      <div className="space-y-6 animate-in fade-in">
        ...
        <input type="text" value={newClientForm.razonSocial} onChange={e=>setNewClientForm({...newClientForm, razonSocial: String(e.target.value ||
'').toUpperCase()})} className="w-full bg-slate-100/70 p-4 rounded-2xl font-black text-xs outline-none focus:bg-white focus:border-orange-500 border-2 border-transparent transition-all" />
                  </div>
                  <div>
                    <label className="text-[10px] font-black text-gray-500 uppercase block mb-2 tracking-widest">RIF</label>
                    <input type="text" disabled={!!editingClientId} value={newClientForm.rif} 
onChange={e=>setNewClientForm({...newClientForm, rif: String(e.target.value || '').toUpperCase()})} className="w-full bg-slate-100/70 p-4 rounded-2xl font-black text-xs outline-none border-2 border-transparent" />
                  </div>
                  <div>
                    <label className="text-[10px] font-black text-gray-500 uppercase block mb-2 tracking-widest">Teléfono</label>
                    <input type="text" value={newClientForm.telefono} 
onChange={e=>setNewClientForm({...newClientForm, telefono: e.target.value})} className="w-full bg-slate-100/70 p-4 rounded-2xl font-black text-xs outline-none border-2 border-transparent" />
                  </div>
                </div>
      ...
      <label className="text-[10px] font-black text-gray-500 uppercase block mb-2 tracking-widest">Dirección Fiscal</label>
                     <input type="text" value={newClientForm.direccion} onChange={e=>setNewClientForm({...newClientForm, direccion: String(e.target.value ||
'').toUpperCase()})} className="w-full bg-slate-100/70 p-4 rounded-2xl font-black text-xs outline-none focus:bg-white focus:border-orange-500 border-2 border-transparent transition-all" />
                  </div>
                  <div>
                     <label className="text-[10px] font-black text-gray-500 uppercase block mb-2 tracking-widest">Vendedor Asignado</label>
                     <input 
type="text" value={newClientForm.vendedor} onChange={e=>setNewClientForm({...newClientForm, vendedor: String(e.target.value || '').toUpperCase()})} className="w-full bg-slate-100/70 p-4 rounded-2xl font-black text-xs outline-none focus:bg-white focus:border-orange-500 border-2 border-transparent transition-all" />
                  </div>
                </div>
                <div className="flex justify-end pt-4">
                  <button type="submit" className="bg-black text-white px-10 py-4 rounded-2xl font-black text-[10px] uppercase 
shadow-xl hover:bg-slate-800 transition-all">GUARDAR DIRECTORIO</button>
                </div>
              </form>
            </div>
...
text-[10px] text-gray-400 tracking-widest"><th className="py-4 px-4">RIF</th><th className="py-4 px-4 w-1/2">Razón Social</th><th className="py-4 px-4">Contacto</th><th className="py-4 px-4 text-center">Acciones</th></tr></thead><tbody className="divide-y divide-gray-100 text-black">{(clients ||
[]).map(c => {
        ...
        {ventasView === 'facturacion' && (
          <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden animate-in fade-in">
             <div className="px-8 py-6 border-b bg-gray-50 flex justify-between items-center"><h2 className="text-xl font-black text-black uppercase flex items-center gap-3 tracking-tighter"><Receipt className="text-orange-500" size={24}/> Facturación de Venta</h2><div className="flex gap-2"><button onClick={()=>setShowGeneralInvoicesReport(true)} className="bg-white border-2 border-gray-100 text-gray-700 px-5 py-2.5 rounded-xl text-[10px] font-black uppercase shadow-sm hover:bg-gray-50 transition-colors">REPORTE GENERAL</button><button onClick={()=>{setShowNewInvoicePanel(!showNewInvoicePanel); 
setNewInvoiceForm(initialInvoiceForm);}} className="bg-black text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase shadow-md hover:bg-slate-800 transition-colors">{showNewInvoicePanel ?
'CANCELAR' : 'NUEVA FACTURA'}</button></div></div>
             {showNewInvoicePanel && (
                <div className="p-8 bg-gray-50/50 border-b">
...
className="md:col-span-2">
                        ...
                        {(clients ||
[]).map(c=><option key={c?.rif} value={c?.rif}>{c?.name}</option>)}
                        </select>
                      </div>
                      <div>
                        <label className="text-[10px] font-black text-gray-600 uppercase mb-2 
block tracking-widest">Base (USD)</label>
             ...
             <div className="p-8"><div className="relative 
max-w-2xl mb-8"><Search className="absolute left-4 top-4 text-gray-400" size={18} /><input type="text" placeholder="BUSCAR FACTURA O CLIENTE..." value={invoiceSearchTerm} onChange={e=>setInvoiceSearchTerm(e.target.value)} className="w-full pl-12 pr-4 py-3.5 border-2 border-gray-100 bg-gray-50/50 rounded-2xl text-xs font-black uppercase outline-none focus:bg-white text-black" /></div><div className="overflow-x-auto"><table className="w-full text-left whitespace-nowrap"><thead className="bg-white border-b-2 border-gray-100"><tr className="uppercase font-black text-[10px] text-gray-400 tracking-widest"><th className="py-4 px-4 text-black">Doc / Fecha</th><th className="py-4 px-4 text-black">Cliente</th><th className="py-4 px-4 text-right text-black">Total USD</th><th className="py-4 px-4 text-center text-black">Acciones</th></tr></thead><tbody className="divide-y">{(invoices ||
[]).map(inv=>{
        ...
        {ventasView === 'requisiciones' && (
          <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden animate-in fade-in">
             <div className="px-8 py-6 border-b bg-white flex justify-between items-center"><h2 className="text-xl font-black text-black uppercase flex items-center gap-3 tracking-tighter"><FileText className="text-orange-500" size={24}/> REQUISICIONES OP</h2><div className="flex gap-2"><button onClick={()=>setShowReqReport(true)} className="bg-white border-2 border-gray-100 text-gray-700 px-6 
py-3 rounded-2xl text-[10px] font-black uppercase shadow-sm hover:bg-gray-50 transition-colors">REPORTE GENERAL</button><button onClick={()=>{setShowNewReqPanel(!showNewReqPanel);setNewReqForm(initialReqForm);setEditingReqId(null);}} className="bg-black text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase shadow-md hover:bg-gray-800 transition-all">{showNewReqPanel ?
             ...
             {showNewReqPanel && (
                <div className="p-8 bg-gray-50/50 border-b">
                  <form onSubmit={handleCreateRequirement} className="bg-white p-10 rounded-3xl border border-gray-100 shadow-sm space-y-6">
                    <div className="flex justify-between items-center border-b pb-3 mb-6"><h3 className="text-sm font-black uppercase text-black">{editingReqId ? 'EDITAR ORDEN' : 'NUEVA ORDEN'}</h3><span 
className="bg-orange-100 text-orange-800 px-4 py-2 rounded-xl font-black text-[10px]">CORRELATIVO: {editingReqId ? String(editingReqId).replace('OP-','').padStart(5,'0') : generateReqId().replace('OP-','').padStart(5,'0')}</span></div>
          ...
          <label className="text-[10px] font-bold text-gray-500 uppercase block mb-2 tracking-widest">Cliente del Directorio</label>
                          <select required value={newReqForm.client} onChange={e=>handleReqFormChange('client', e.target.value)} className="w-full bg-white border-2 border-gray-200 rounded-2xl p-4 font-black text-xs text-black outline-none focus:border-orange-500">
                            <option value="">Seleccione...</option>{(clients ||
[]).map(c=><option key={c?.rif} value={c?.name}>{c?.name}</option>)}
                          </select>
                        </div>
      ...
      <div><label className="text-[10px] font-bold text-gray-500 uppercase block mb-2 tracking-widest">Ancho (cm)</label><input type="number" step="0.1" value={newReqForm.ancho} onChange={e=>handleReqFormChange('ancho', e.target.value)} className="w-full border-2 border-gray-200 rounded-2xl p-5 font-black text-lg text-center text-black" /></div>
                      ...
                      <div className="space-y-4">
                        <div><label className="text-[10px] font-bold text-gray-500 uppercase block mb-2 tracking-widest">Largo (cm)</label><input type="number" step="0.1" value={newReqForm.largo} onChange={e=>handleReqFormChange('largo', e.target.value)} disabled={newReqForm.tipoProducto === 'TERMOENCOGIBLE'} className="w-full border-2 border-gray-200 rounded-2xl p-5 font-black text-lg 
text-center text-black disabled:bg-gray-100 disabled:opacity-50" /></div>
...
text-black uppercase flex items-center gap-3 tracking-tighter"><Calculator className="text-orange-500" size={24}/> Simulador de Producción</h2>
    ...
    <style>{`@media print { @page { size: landscape; margin: 5mm; } .print-text-xs { font-size: 8px !important;
} }`}</style>
               {/* PANEL DE CONTROLES */}
               <div data-html2canvas-ignore="true" className="lg:col-span-4 border-r border-gray-200 bg-gray-50 p-8 print:hidden space-y-8">
                 ...
                 <div>
                    ...
                    <h3 className="text-xs font-black uppercase text-black mb-4 border-b border-gray-200 pb-2">1.
Variables de Mezcla</h3>
                     <div className="space-y-4">
                        <div>
                          <label className="text-[10px] font-bold text-gray-500 uppercase block mb-1">Total a Preparar (KG)</label>
                 
         <input type="number" value={calcInputs?.mezclaTotal ||
''} onChange={(e) => handleCalcChange('mezclaTotal', e.target.value)} className="w-full border-2 border-gray-200 rounded-xl p-3 text-sm font-black outline-none focus:border-orange-500 text-center text-blue-600" />
                        </div>
                     </div>
                 </div>
                ...
                <div>
                     <div className="flex justify-between items-end border-b border-gray-200 pb-2 mb-4">
                       <h3 className="text-xs font-black uppercase text-black">2.
Fórmula de MP</h3>
...
setCalcInputs({ ...calcInputs, ingredientes: newIngs });
          ...
          <option value="">SELECCIONE MATERIA PRIMA...</option>
                                ...
                                {!(inventory || []).find(i 
=> i?.id === 'MP-0240') && <option value="MP-0240">MP-0240 - PEBD 240 (ESENTTIA)</option>}
                                {!(inventory || []).find(i => i?.id === 'MP-11PG4') && <option value="MP-11PG4">MP-11PG4 - LINEAL 11PG4 (METALOCENO)</option>}
                                {!(inventory ||
[]).find(i => i?.id === 'MP-3003') && <option value="MP-3003">MP-3003 - PEBD 3003 (BAPOLENE)</option>}
                              </select>
 ...
 <div className="flex gap-2 mt-1">
            ...
            <input type="number" value={ing?.pct ||
''} onChange={(e) => updateCalcIng(ing?.id, 'pct', e.target.value)} className="w-full text-xs font-black text-center outline-none bg-gray-50 rounded p-1 border border-gray-100 text-black" />
                                </div>
                                <div className="w-1/2">
                  
                 <label className="text-[8px] font-bold text-gray-400 uppercase">Costo ($/KG)</label>
                                   <input type="number" step="0.01" value={ing?.costo ||
''} onChange={(e) => updateCalcIng(ing?.id, 'costo', e.target.value)} className="w-full text-xs font-black text-center outline-none bg-gray-50 rounded p-1 border border-gray-100 text-black" />
                                </div>
                              </div>
...
uppercase ${(calcInputs?.ingredientes || []).reduce((a,b)=>a+(b?.pct||0),0) !== 100 ? 'text-red-500' : 'text-green-500'}`}>Total Fórmula: {(calcInputs?.ingredientes || []).reduce((a,b)=>a+(b?.pct||0),0)}%</span>
                     </div>
                 </div>
         ...
         <h3 className="text-xs font-black uppercase text-black mb-4 border-b border-gray-200 pb-2">3.
Proyección de Merma Global</h3>
                     <div className="space-y-3">
                        <div className="flex items-center justify-between gap-4">
                          <label className="text-[9px] font-bold text-gray-500 uppercase flex-1">Merma Global Esperada (%)</label>
             
             <input type="number" step="0.1" value={calcInputs?.mermaGlobalPorc ||
''} onChange={(e) => handleCalcChange('mermaGlobalPorc', e.target.value)} className="w-24 border-2 border-gray-200 rounded-lg p-2 text-xs font-black text-center text-red-500" />
                        </div>
                     </div>
                 </div>
                ...
                <div>
                     <h3 className="text-xs font-black uppercase text-black mb-4 border-b border-gray-200 pb-2">4.
Parámetros del Producto Final</h3>
                     <div className="space-y-3">
                        <div>
                          <label className="text-[9px] font-bold text-gray-500 uppercase block mb-1">Tipo de Producto</label>
                 
         <select value={calcInputs?.tipoProducto ||
'BOLSAS'} onChange={e=>setCalcInputs({...calcInputs, tipoProducto: e.target.value})} className="w-full border-2 border-gray-200 rounded-lg p-2 text-xs font-black text-center text-blue-600 outline-none">
                            <option value="BOLSAS">BOLSAS / EMPAQUES</option>
                            <option value="TERMOENCOGIBLE">TERMOENCOGIBLE</option>
                          </select>
    ...
    <label className="text-[8px] font-bold text-gray-400 uppercase">ANCHO (CM)</label>
                            <input type="number" step="0.1" value={calcInputs?.ancho ||
''} onChange={e=>setCalcInputs({...calcInputs, ancho: e.target.value})} className="w-full border p-2 text-xs text-center font-bold" />
                          </div>
                          <div>
                            <label className="text-[8px] font-bold text-gray-400 uppercase">FUELLES (CM)</label>
     
                       <input type="number" step="0.1" disabled={calcInputs?.tipoProducto === 'TERMOENCOGIBLE'} value={calcInputs?.fuelles ||
''} onChange={e=>setCalcInputs({...calcInputs, fuelles: e.target.value})} className="w-full border p-2 text-xs text-center font-bold disabled:bg-gray-100" />
                          </div>
                          <div>
                            <label className="text-[8px] font-bold text-gray-400 uppercase">LARGO (CM)</label>
    
                        <input type="number" step="0.1" disabled={calcInputs?.tipoProducto === 'TERMOENCOGIBLE'} value={calcInputs?.largo ||
''} onChange={e=>setCalcInputs({...calcInputs, largo: e.target.value})} className="w-full border p-2 text-xs text-center font-bold disabled:bg-gray-100" />
                          </div>
                          <div>
                            <label className="text-[8px] font-bold text-gray-400 uppercase">MICRAS</label>
     
                       <input type="number" step="0.001" value={calcInputs?.micras ||
''} onChange={e=>setCalcInputs({...calcInputs, micras: e.target.value})} className="w-full border p-2 text-xs text-center font-bold" />
                          </div>
                        </div>
                     </div>
                 </div>
               ...
               {/* TABLA DE RESULTADO (VISTA IMPRIMIBLE CON COSTOS) */}
               <div className="lg:col-span-8 p-10 bg-white print:w-full print:p-0">
                  <div className="hidden print:block mb-6">
                     <ReportHeader />
   
                  <h1 className="text-xl font-black text-black uppercase border-b-2 border-orange-500 pb-1 mt-2">PROYECCIÓN DE PRODUCCIÓN</h1>
                  </div>
       ...
       <div className={"bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden " + (recipeEditReqId ?
...
[]); 
                            if (!r?.viewedByPlanta) {
                               updateDoc(getDocRef('requirements', r.id), { viewedByPlanta: true }).catch(e=>console.error(e));
}
       ...
       <div className="lg:col-span-1 bg-white rounded-3xl shadow-xl border border-gray-200 p-8 animate-in slide-in-from-right">
             ...
             <p className="text-[10px] font-black text-orange-800 uppercase">OP N°: {String(recipeEditReqId).replace('OP-', '').padStart(5, '0')}</p>
                     <p className="text-[10px] font-black text-orange-800 uppercase">CLIENTE: {(requirements ||
[]).find(r=>r?.id===recipeEditReqId)?.client}</p>
                  </div>
             ...
             <div>TIPO: {(requirements || []).find(r=>r?.id===recipeEditReqId)?.tipoProducto}</div>
                     ...
                     <div>VENDEDOR: {(requirements || []).find(r=>r?.id===recipeEditReqId)?.vendedor ||
'S/N'}</div>
                  </div>
                </div>
           ...
           <div className="flex gap-3 items-end">
                    <div className="flex-1">
                      ...
                      <input type="number" step="0.001" value={newIngQty} onChange={e=>setNewIngQty(e.target.value)} className="w-full border-2 border-gray-200 rounded-xl p-3 text-xs font-black outline-none 
focus:bg-white focus:border-orange-500 text-black" />
                    </div>
...
[]).map((ing, idx) => (
             ...
             {ing?.percentage !== null && <span className="text-[9px] font-bold text-gray-500 mt-0.5">PORCENTAJE: {ing?.percentage}%</span>}
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-[10px] font-black text-orange-600 bg-orange-100 px-2 py-1 
rounded-lg">{formatNum(ing?.totalQty)} KG</span>
...
return (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in slide-in-from-bottom-8">
                  <div className="lg:col-span-1 space-y-6">
                    <div className="bg-black rounded-3xl shadow-xl p-8 text-white relative overflow-hidden"><div className="absolute -right-6 -bottom-6 opacity-10"><Factory size={160}/></div><div className="relative z-10"><span className="bg-orange-500 text-white px-3 py-1.5 rounded-lg text-[9px] font-black uppercase mb-4 inline-block shadow-sm">EN PRODUCCIÓN</span><h2 className="text-4xl font-black uppercase tracking-tighter mb-2">#{String(req.id).replace('OP-', '').padStart(5, '0')}</h2><p className="text-sm font-bold text-gray-300 
uppercase leading-relaxed mb-6 border-b border-gray-700 pb-6">{req.client}<br/><span className="text-orange-400 text-lg">{req.desc}</span></p><div className="bg-gray-800 p-5 rounded-2xl border border-gray-700 shadow-inner"><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1 tracking-widest">META A PRODUCIR:</p><p className="text-3xl font-black text-white">{formatNum(req.requestedKg)} KG</p></div></div></div>
                    <div className="bg-white rounded-3xl shadow-sm border border-gray-200 p-3 space-y-2">{[{ id: 'extrusion', label: '1.
Extrusión' }, { id: 'impresion', label: '2. Impresión' }, { id: 'sellado', label: '3.
Sellado' }].map(tab => (<button key={tab.id} onClick={() => {setActivePhaseTab(tab.id); setPhaseForm({...initialPhaseForm, date: getTodayDate()});}} className={`w-full flex justify-between items-center p-5 rounded-2xl text-[10px] font-black uppercase transition-all ${activePhaseTab === tab.id ? 'bg-orange-50 text-orange-700 border-2 border-orange-200' : 'text-gray-500 hover:bg-gray-50'}`}><span>{tab.label}</span>{req.production?.[tab.id]?.isClosed && <CheckCircle size={18} className="text-green-500"/>}</button>))}</div>
                  </div>
                  
                  <div className="lg:col-span-2 bg-white rounded-3xl shadow-sm border border-gray-200 p-8 lg:p-10 text-black">
  
                  <div className="border-b-2 border-gray-100 pb-6 mb-8 flex justify-between items-center"><h3 className="text-2xl font-black uppercase text-black tracking-tighter">Fase: {activePhaseTab.toUpperCase()}</h3><button onClick={()=>{setSelectedPhaseReqId(null);
setPhaseForm(initialPhaseForm);}} className="bg-gray-100 p-2.5 rounded-xl text-gray-500 hover:text-black"><X size={18}/></button></div>
                    {cPhase.batches && cPhase.batches.length > 0 && (<div className="mb-8 overflow-hidden rounded-2xl border border-gray-200"><table className="w-full text-center text-xs"><thead className="bg-gray-50 border-b border-gray-200"><tr className="uppercase font-black text-[9px] text-gray-500 tracking-widest"><th className="p-3 border-r border-gray-200">Fecha</th><th className="p-3 border-r border-gray-200">Producido</th><th className="p-3 border-r border-gray-200">Merma</th><th className="p-3">Acción</th></tr></thead><tbody className="divide-y divide-gray-100 text-black">{(cPhase.batches || []).map(b => (<tr key={b?.id} className="hover:bg-gray-50"><td className="p-3 border-r border-gray-200 font-bold">{b?.date}</td><td className="p-3 border-r border-gray-200 font-black text-green-600">{formatNum(b?.producedKg)} kg</td><td className="p-3 border-r border-gray-200 font-black text-red-500">{formatNum(b?.mermaKg)} kg</td><td className="p-3 text-center flex justify-center gap-2"><button onClick={() => handleEditBatch(req.id, activePhaseTab, b?.id)} className="bg-blue-50 text-blue-500 hover:bg-blue-500 hover:text-white transition-colors p-1.5 
rounded-lg" title="Modificar"><Edit size={14}/></button><button onClick={() => handleDeleteBatch(req.id, activePhaseTab, b?.id)} className="bg-red-50 text-red-500 hover:bg-red-500 hover:text-white transition-colors p-1.5 rounded-lg" title="Eliminar"><Trash2 size={14}/></button></td></tr>))}</tbody></table></div>)}
                    {cPhase.isClosed ?
(<div className="text-center py-10 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200 shadow-inner"><CheckCircle size={56} className="text-green-500 mx-auto mb-6"/><h4 className="text-xl font-black text-black uppercase tracking-widest">Esta Fase se encuentra cerrada</h4><p className="text-[10px] font-bold text-gray-400 uppercase mt-2 mb-6">Ya no se permiten reportes parciales en esta etapa.</p><button onClick={async () => {
                        const newProd = { ...(req.production || {}), [activePhaseTab]: { ...cPhase, isClosed: false } };
                        ...
                        setDialog({title: 'Fase Reabierta', text: 'Ahora puedes modificar o añadir lotes.', type: 'alert'});
                      }} className="bg-orange-500 text-white px-6 py-2 rounded-xl text-[10px] font-black uppercase shadow-md hover:bg-orange-600 transition-all flex items-center gap-2 mx-auto"><Edit size={14}/> REABRIR FASE PARA EDITAR</button></div>) : (
                      <form onSubmit={handleSavePhase} className="space-y-8">
                        <div className="flex gap-4 items-center"><label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Fecha Reporte:</label><input type="date" value={phaseForm?.date || getTodayDate()} onChange={e=>setPhaseForm({...phaseForm, date: 
e.target.value})} className="border-2 border-gray-200 rounded-xl p-2 font-black text-xs outline-none focus:border-orange-500" /></div>
                ...
                <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200 shadow-inner mb-6">
                             ...
                             <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                                ...
                                <div><label className="text-[9px] font-black text-gray-500 uppercase">Operador</label><input type="text" value={phaseForm?.operadorExt ||
''} onChange={e=>setPhaseForm({...phaseForm, operadorExt: String(e.target.value || '').toUpperCase()})} className="w-full border p-2 text-xs rounded-lg uppercase outline-none focus:border-orange-500"/></div>
                                <div>
                                  <label className="text-[9px] font-black text-gray-500 uppercase">Tratado</label>
                 
                 <select value={phaseForm?.tratado ||
''} onChange={e=>setPhaseForm({...phaseForm, tratado: e.target.value})} className="w-full border p-2 text-xs rounded-lg uppercase outline-none focus:border-orange-500">
                                    <option value="">Seleccione...</option><option value="1">Tratado 1</option><option value="2">Tratado 2</option>
                                  </select>
              
                  </div>
                                <div><label className="text-[9px] font-black text-gray-500 uppercase">Motor Principal</label><input type="text" value={phaseForm?.motorExt ||
''} onChange={e=>setPhaseForm({...phaseForm, motorExt: String(e.target.value || '').toUpperCase()})} className="w-full border p-2 text-xs rounded-lg uppercase outline-none focus:border-orange-500"/></div>
                                <div><label className="text-[9px] font-black text-gray-500 uppercase">Ventilador</label><input type="text" value={phaseForm?.ventilador ||
''} onChange={e=>setPhaseForm({...phaseForm, ventilador: String(e.target.value || '').toUpperCase()})} className="w-full border p-2 text-xs rounded-lg uppercase outline-none focus:border-orange-500"/></div>
                                <div><label className="text-[9px] font-black text-gray-500 uppercase">Jalador</label><input type="text" value={phaseForm?.jalador ||
''} onChange={e=>setPhaseForm({...phaseForm, jalador: String(e.target.value || '').toUpperCase()})} className="w-full border p-2 text-xs rounded-lg uppercase outline-none focus:border-orange-500"/></div>
                             </div>
                             <div className="border-t border-gray-200 pt-4">
                          
      <p className="text-[9px] font-black text-gray-500 uppercase mb-2">Zonas de Calor (1 al 6)</p>
                               <div className="grid grid-cols-6 gap-2 mb-4">
                                 <input type="text" placeholder="Z1" value={phaseForm?.zona1 ||
''} onChange={e=>setPhaseForm({...phaseForm, zona1: e.target.value})} className="border p-2 text-xs rounded-lg text-center" />
                                 <input type="text" placeholder="Z2" value={phaseForm?.zona2 ||
''} onChange={e=>setPhaseForm({...phaseForm, zona2: e.target.value})} className="border p-2 text-xs rounded-lg text-center" />
                                 <input type="text" placeholder="Z3" value={phaseForm?.zona3 ||
''} onChange={e=>setPhaseForm({...phaseForm, zona3: e.target.value})} className="border p-2 text-xs rounded-lg text-center" />
                                 <input type="text" placeholder="Z4" value={phaseForm?.zona4 ||
''} onChange={e=>setPhaseForm({...phaseForm, zona4: e.target.value})} className="border p-2 text-xs rounded-lg text-center" />
                                 <input type="text" placeholder="Z5" value={phaseForm?.zona5 ||
''} onChange={e=>setPhaseForm({...phaseForm, zona5: e.target.value})} className="border p-2 text-xs rounded-lg text-center" />
                                 <input type="text" placeholder="Z6" value={phaseForm?.zona6 ||
''} onChange={e=>setPhaseForm({...phaseForm, zona6: e.target.value})} className="border p-2 text-xs rounded-lg text-center" />
                               </div>
                               <p className="text-[9px] font-black text-gray-500 uppercase mb-2">Cabezales</p>
                        
       <div className="grid grid-cols-2 gap-4">
                                 <div><label className="text-[8px] font-bold text-gray-400">Cabezal A</label><input type="text" value={phaseForm?.cabezalA ||
''} onChange={e=>setPhaseForm({...phaseForm, cabezalA: e.target.value})} className="w-full border p-2 text-xs rounded-lg" /></div>
                                 <div><label className="text-[8px] font-bold text-gray-400">Cabezal B</label><input type="text" value={phaseForm?.cabezalB ||
''} onChange={e=>setPhaseForm({...phaseForm, cabezalB: e.target.value})} className="w-full border p-2 text-xs rounded-lg" /></div>
                               </div>
                             </div>
                          </div>
         ...
         <h4 className="text-[10px] font-black text-gray-600 uppercase mb-4 flex items-center gap-2"><Settings2 size={16}/> Parámetros Técnicos - Impresión</h4>
                             <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <div><label className="text-[9px] font-black text-gray-500 uppercase">Operador</label><input type="text" value={phaseForm?.operadorImp || ''} onChange={e=>setPhaseForm({...phaseForm, operadorImp: String(e.target.value || 
'').toUpperCase()})} className="w-full border p-2 text-xs rounded-lg uppercase outline-none focus:border-orange-500"/></div>
                                <div><label className="text-[9px] font-black text-gray-500 uppercase">KG Recibidos</label><input type="number" value={phaseForm?.kgRecibidosImp ||
''} onChange={e=>setPhaseForm({...phaseForm, kgRecibidosImp: e.target.value})} className="w-full border p-2 text-xs rounded-lg uppercase outline-none focus:border-orange-500"/></div>
                                <div><label className="text-[9px] font-black text-gray-500 uppercase">Cant.
Colores</label><input type="number" value={phaseForm?.cantColores || ''} onChange={e=>setPhaseForm({...phaseForm, cantColores: e.target.value})} className="w-full border p-2 text-xs rounded-lg uppercase outline-none focus:border-orange-500"/></div>
                                <div><label className="text-[9px] font-black text-gray-500 uppercase">Relación Imp.</label><input type="text" value={phaseForm?.relacionImp ||
''} onChange={e=>setPhaseForm({...phaseForm, relacionImp: String(e.target.value || '').toUpperCase()})} className="w-full border p-2 text-xs rounded-lg uppercase outline-none focus:border-orange-500"/></div>
                                <div><label className="text-[9px] font-black text-gray-500 uppercase">Motor Principal</label><input type="text" value={phaseForm?.motorImp ||
''} onChange={e=>setPhaseForm({...phaseForm, motorImp: String(e.target.value || '').toUpperCase()})} className="w-full border p-2 text-xs rounded-lg uppercase outline-none focus:border-orange-500"/></div>
                                <div><label className="text-[9px] font-black text-gray-500 uppercase">Tensores (1 y 2)</label><input type="text" value={phaseForm?.tensores ||
''} onChange={e=>setPhaseForm({...phaseForm, tensores: String(e.target.value || '').toUpperCase()})} placeholder="Ej: 15 / 20" className="w-full border p-2 text-xs rounded-lg uppercase outline-none focus:border-orange-500"/></div>
                                <div><label className="text-[9px] font-black text-gray-500 uppercase">Temperatura</label><input type="text" value={phaseForm?.tempImp ||
''} onChange={e=>setPhaseForm({...phaseForm, tempImp: String(e.target.value || '').toUpperCase()})} className="w-full border p-2 text-xs rounded-lg uppercase outline-none focus:border-orange-500"/></div>
                                <div><label className="text-[9px] font-black text-gray-500 uppercase">Cant.
Solvente</label><input type="text" value={phaseForm?.solvente || ''} onChange={e=>setPhaseForm({...phaseForm, solvente: String(e.target.value || '').toUpperCase()})} className="w-full border p-2 text-xs rounded-lg uppercase outline-none focus:border-orange-500"/></div>
                             </div>
                          </div>
                        )}
                    ...
                    {activePhaseTab === 'sellado' && (
                          <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200 shadow-inner mb-6">
                             <h4 className="text-[10px] font-black text-gray-600 uppercase mb-4 flex items-center gap-2"><Settings2 size={16}/> Parámetros Técnicos - Sellado 
y Corte</h4>
                             <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                                <div><label className="text-[9px] font-black text-gray-500 uppercase">Operador</label><input type="text" value={phaseForm?.operadorSel || ''} onChange={e=>setPhaseForm({...phaseForm, operadorSel: String(e.target.value ||
'').toUpperCase()})} className="w-full border p-2 text-xs rounded-lg uppercase outline-none focus:border-orange-500"/></div>
                                <div><label className="text-[9px] font-black text-gray-500 uppercase">KG Recibidos</label><input type="number" value={phaseForm?.kgRecibidosSel ||
''} onChange={e=>setPhaseForm({...phaseForm, kgRecibidosSel: e.target.value})} className="w-full border p-2 text-xs rounded-lg uppercase outline-none focus:border-orange-500"/></div>
                                <div>
                                  <label className="text-[9px] font-black text-gray-500 uppercase">Impresa</label>
                   
               <select value={phaseForm?.impresa ||
'NO'} onChange={e=>setPhaseForm({...phaseForm, impresa: e.target.value})} className="w-full border p-2 text-xs rounded-lg uppercase outline-none focus:border-orange-500">
                     ...
                     <select value={phaseForm?.tipoSello ||
'Sello FC'} onChange={e=>setPhaseForm({...phaseForm, tipoSello: e.target.value})} className="w-full border p-2 text-xs rounded-lg uppercase outline-none focus:border-orange-500">
                    ...
                    <div>
                                  ...
                                  <div className="flex 
gap-2">
                                    <input type="text" placeholder="Cabezal A" value={phaseForm?.tempCabezalA ||
''} onChange={e=>setPhaseForm({...phaseForm, tempCabezalA: e.target.value})} className="w-1/2 border p-2 text-xs rounded-lg text-center" />
                                    <input type="text" placeholder="Cabezal B" value={phaseForm?.tempCabezalB ||
''} onChange={e=>setPhaseForm({...phaseForm, tempCabezalB: e.target.value})} className="w-1/2 border p-2 text-xs rounded-lg text-center" />
                                  </div>
                                </div>
        ...
        <div>
                         ...
                         <input type="text" placeholder="Piso A" value={phaseForm?.tempPisoA ||
''} onChange={e=>setPhaseForm({...phaseForm, tempPisoA: e.target.value})} className="w-1/2 border p-2 text-xs rounded-lg text-center" />
                                    <input type="text" placeholder="Piso B" value={phaseForm?.tempPisoB ||
''} onChange={e=>setPhaseForm({...phaseForm, tempPisoB: e.target.value})} className="w-1/2 border p-2 text-xs rounded-lg text-center" />
                                  </div>
                                </div>
        ...
        <div className="col-span-2">
                                  ...
                                  <input type="text" placeholder="Ej: S1:100 / S2:90 / S3:100 / 
S4:110" value={phaseForm?.velServo || ''} onChange={e=>setPhaseForm({...phaseForm, velServo: String(e.target.value || '').toUpperCase()})} className="w-full border p-2 text-xs rounded-lg outline-none focus:border-orange-500"/>
                                </div>
                             </div>
                        
     <div className="grid grid-cols-2 gap-4 pt-4 mt-4 border-t border-gray-200">
                                <div><label className="text-[9px] font-black text-gray-500 uppercase mb-1 block">Cant.
Producida ({req?.tipoProducto === 'TERMOENCOGIBLE' ? 'Kilos' : 'Millares'})</label><input type="number" step="0.01" value={phaseForm?.millaresProd ||
''} onChange={e=>setPhaseForm({...phaseForm, millaresProd: e.target.value})} className="w-full border p-2 text-xs rounded-lg outline-none focus:border-orange-500"/></div>
                                <div><label className="text-[9px] font-black text-gray-500 uppercase mb-1 block">Troquel</label><input type="text" value={phaseForm?.troquelSel ||
''} onChange={e=>setPhaseForm({...phaseForm, troquelSel: String(e.target.value || '').toUpperCase()})} className="w-full border p-2 text-xs rounded-lg outline-none focus:border-orange-500"/></div>
                             </div>
                          </div>
                        )}

         
                <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200"><h4 className="text-[10px] font-black text-gray-600 uppercase mb-4 flex items-center gap-2"><Box size={16}/> Insumos Consumidos</h4><div className="flex gap-3 mb-6"><select value={phaseIngId} onChange={e=>setPhaseIngId(e.target.value)} className="flex-1 border-2 border-gray-200 rounded-xl p-3.5 font-black text-xs outline-none focus:border-orange-500">{renderPhaseInventoryOptions()}</select><input type="number" step="0.01" value={phaseIngQty} onChange={e=>setPhaseIngQty(e.target.value)} placeholder="Cant" className="w-32 border-2 border-gray-200 rounded-xl p-3.5 text-xs font-black text-center outline-none focus:border-orange-500" /><button type="button" onClick={handleAddPhaseIng} className="bg-black text-white px-5 rounded-xl shadow-md transition-all hover:bg-slate-800"><Plus size={20}/></button></div><ul className="space-y-3">{(phaseForm?.insumos ||
[]).map((ing, idx) => (<li key={idx} className="flex justify-between items-center bg-white p-4 rounded-xl border border-gray-200 shadow-sm"><span className="text-xs font-black uppercase">{(inventory || []).find(i=>i?.id===ing?.id)?.desc || ing?.id}</span><div className="flex items-center gap-4"><span className="text-sm font-black bg-gray-100 px-3 py-1.5 rounded-lg">{ing?.qty}</span><button type="button" onClick={() => setPhaseForm({...phaseForm, insumos: (phaseForm?.insumos || []).filter((_, i) => i !== idx)})} className="text-red-400 hover:text-red-600 transition-colors"><Trash2 size={18}/></button></div></li>))}</ul></div><div className="grid grid-cols-2 gap-4"><div className="bg-green-50 p-4 rounded-2xl border border-green-200 shadow-inner"><label className="text-[9px] font-black text-green-800 uppercase block mb-2 tracking-widest">Producido Bruto (KG)</label><input type="number" step="0.01" value={phaseForm?.producedKg ||
''} onChange={e=>setPhaseForm({...phaseForm, producedKg: e.target.value})} placeholder="0.00 KG" className="w-full border-2 border-green-300 rounded-xl p-3 text-lg font-black text-green-700 text-center outline-none focus:border-green-500" /></div><div className="bg-red-50 p-4 rounded-2xl border border-red-200 shadow-inner"><label className="text-[9px] font-black text-red-800 uppercase block mb-2 tracking-widest">Mermas / Desperdicio (KG)</label><input type="number" step="0.01" value={phaseForm?.mermaKg ||
''} onChange={e=>setPhaseForm({...phaseForm, mermaKg: e.target.value})} placeholder="0.00 KG" className="w-full border-2 border-red-300 rounded-xl p-3 text-lg font-black text-red-700 text-center outline-none focus:border-red-500" /></div></div><div className="flex flex-col md:flex-row gap-4 pt-6 border-t-2 border-gray-100"><button type="submit" name="skip" className="w-full md:w-1/4 bg-gray-100 text-gray-500 font-black py-4 rounded-2xl uppercase text-[9px] border-2 border-gray-200 shadow-sm transition-all hover:bg-gray-200">OMITIR FASE</button><button type="submit" name="partial" className="w-full md:w-2/4 bg-blue-50 text-blue-600 font-black py-4 rounded-2xl uppercase text-[9px] border-2 border-blue-200 flex justify-center items-center gap-2 shadow-sm transition-all hover:bg-blue-100"><Plus size={16}/> GUARDAR REPORTE PARCIAL</button><button type="submit" name="close" className="w-full md:w-1/4 bg-black text-white font-black py-4 rounded-2xl uppercase text-[9px] flex justify-center items-center gap-2 shadow-xl hover:bg-slate-800 transition-all"><CheckCircle size={16}/> CERRAR FASE DEFINITIVA</button></div>
       ...
       <div className="overflow-x-auto"><table className="w-full text-left text-sm whitespace-nowrap"><thead className="bg-gray-50 border-b border-gray-200"><tr><th className="p-4 text-[10px] font-black uppercase tracking-widest">OP N°</th><th className="p-4 text-[10px] font-black uppercase tracking-widest">Cliente / Producto</th><th className="p-4 text-right text-[10px] font-black uppercase tracking-widest">KG Finales</th><th className="p-4 text-center text-[10px] font-black uppercase tracking-widest">Acciones</th></tr></thead><tbody className="divide-y divide-gray-100 text-black">{(completedOrders ||
[]).map(req => (<tr key={req?.id} className="hover:bg-gray-50 group"><td className="p-4 font-black text-orange-500 text-lg">#{String(req?.id).replace('OP-', '').padStart(5, '0')}</td><td className="p-4 font-black uppercase text-xs">{req?.client}<br/><span className="text-[10px] font-bold text-gray-400">{req?.desc}</span></td><td className="p-4 text-right font-black text-green-600 text-lg">{formatNum((req?.production?.sellado?.batches || []).reduce((a,b)=>a+parseNum(b?.producedKg),0) || (req?.production?.extrusion?.batches || []).reduce((a,b)=>a+parseNum(b?.producedKg),0) || 0)} KG</td><td className="p-4 text-center"><button onClick={()=>setShowFiniquito(req?.id)} className="bg-black text-white px-8 py-3 rounded-2xl text-[9px] font-black uppercase hover:bg-gray-800 shadow-md transition-all">GENERAR FINIQUITO</button></td></tr>))}</tbody></table></div>
          </div>
        )}
      </div>
    );
...
if (batches.length === 0) return 'PENDIENTE';
       ...
       return batches[0]?.date || 'PENDIENTE';
    };
...
if (batches.length === 0) return 'COMPLETADO';
       ...
       return batches[0]?.date || getTodayDate();
    };
return (
      <div id="pdf-content" className="bg-white p-6 print:p-0 min-h-screen text-black"><style>{`@media print { @page { size: portrait; margin: 5mm; } }`}</style>
        ...
        <div className="grid grid-cols-3 text-[9px] font-bold uppercase mb-2 border-b-2 border-black pb-2">
           <div>
              ...
              <p className="mb-1"><span className="w-16 inline-block font-black pr-1">OP:</span> #{String(req?.id).replace('OP-', 
'').padStart(5, '0')}</p>
              <p><span className="w-16 inline-block font-black pr-1">TIPO:</span> {req?.tipoProducto ||
'N/A'}</p>
           </div>
           <div>
              <p className="mb-1"><span className="w-20 inline-block font-black pr-1">EMISIÓN OP:</span> {req?.fecha ||
getSafeDate(req?.timestamp)}</p>
           ...
           <div>
              ...
              <p><span className="w-24 inline-block font-black pr-1">FECHA SALIDA:</span> <span className={req?.status === 'COMPLETADO' ?
'text-green-600' : 'text-gray-500'}>{getFechaSalida(req)}</span></p>
           </div>
        </div>
       ...
       <tbody className="divide-y divide-gray-100">
          ...
          <div className="grid grid-cols-4 gap-2 text-center text-[9px] font-black uppercase border-t-2 border-black pt-2 bg-gray-50">
            <div>ANCHO<br/><span 
className="text-sm text-orange-600">{req?.ancho} CM</span></div>
            <div>FUELLES<br/><span className="text-sm text-orange-600">{req?.fuelles ||
'0'} CM</span></div>
        ...
        <div className="border-2 border-black rounded-xl mb-2 overflow-hidden">
           ...
           <div className="p-2 text-[8px] font-bold uppercase grid grid-cols-2 
gap-y-2">
...
className="col-span-2 flex justify-between">
                 ...
                 <div className="font-black">SELLO FC _____ &nbsp;&nbsp;
SELLO FR _____ &nbsp;&nbsp; SELLO PC _____</div>
              </div>
   ...
   <div><span className="font-black pr-1">TEMP CABEZAL B:</span> ____________</div>
            ...
            <div className="col-span-2 grid grid-cols-2 gap-y-1 pt-1">
                 <div><span className="font-black pr-1">CANT.
PRODUCIDA (KG):</span> _______________</div>
                 <div><span className="font-black pr-1">TROQUEL:</span> __________________</div>
                 <div><span className="font-black pr-1">CANT.
PRODUCIDA MILLARES:</span> ___________</div>
...
extBatches.forEach(b => {
        (b?.insumos || []).forEach(ing => {
            ...
            if(existing) { existing.qty += (ing?.qty || 0); }
            else { mpConsumida.push({...ing}); }
        });
    });
const totalMezclaPreparada = mpConsumida.reduce((sum, item) => sum + (item?.qty || 0), 0) || parseNum(req?.requestedKg);
...
const unitFinal = isTermo ? 'KG' : 'Millares';
    ...
    return (
      <div id="pdf-content" className="bg-white p-12 print:p-0 min-h-screen text-black shadow-none border-0 font-black uppercase"><style>{`@media print { @page { size: landscape; margin: 5mm; } body { background-color: white !important; } .text-orange-600 { color: #ea580c !important; } }`}</style>
       ...
       <div className="flex justify-between items-end border-b-2 border-black pb-4 mb-6">
        ...
        <div>
                 <h1 className="text-lg font-black uppercase tracking-widest text-black">REPORTE FINAL DE PRODUCCIÓN</h1>
...
<div className="col-span-2"><span className="text-gray-500 block mb-1">PRODUCTO:</span> {req?.desc}</div>
    ...
    <tr><td colSpan="4" className="p-2 font-black uppercase text-[11px] text-orange-600 bg-orange-50">1.
MATERIA PRIMA CONSUMIDA</td></tr>
                 {mpConsumida.length > 0 ?
mpConsumida.map((ing, i) => (
                   <tr key={i} className="hover:bg-gray-50">
                     ...
                     <td className="p-2 
text-center text-gray-500">kg</td>
                     <td className="p-2 text-gray-600">{formatNum(((ing?.qty||0)/totalMezclaPreparada)*100)}% de la mezcla</td>
                   </tr>
                 )) : (
                   <tr><td colSpan="4" className="p-4 text-center text-gray-500 italic">Sin reporte de insumos.</td></tr>
        
         )}
                 <tr className="bg-gray-100 font-black border-y-2 border-gray-300">
                   <td className="p-2 pl-4 text-black">TOTAL MATERIA PRIMA UTILIZADA</td>
                  ...
                  <td className="p-2 text-gray-600">Ingreso real a Planta</td>
                 </tr>

                 <tr><td colSpan="4" className="p-2 pt-4 font-black uppercase text-[11px] text-orange-600 bg-orange-50">2.
RESULTADO FINAL</td></tr>
                 <tr className="bg-black font-black border-y-4 border-orange-500 text-[12px] text-white print:border-black print:bg-gray-200 print:text-black">
                   ...
                   <td className="p-3 text-center text-orange-500 print:text-black">{formatNum(totalFinalUnidades)}</td>
                   <td className="p-3 text-center text-orange-500 print:text-black">{unitFinal}</td>
    
               <td className="p-3 text-gray-400 print:text-gray-700 text-[9px]">Unidades empacadas final</td>
                 </tr>
              </tbody>
           </table>
        </div>
...
className="flex items-center gap-5">
                <div className="flex items-center gap-3 bg-gray-800 px-5 py-2 rounded-2xl border border-gray-700 shadow-inner"><ShieldCheck size={18} className="text-orange-500" /><span className="font-black text-white text-[10px] uppercase leading-none">{appUser?.name}</span></div>
                <button onClick={() => setAppUser(null)} className="text-gray-400 hover:text-white transition-all bg-gray-800 hover:bg-red-600 p-2.5 rounded-2xl border border-gray-700 shadow-lg"><LogOut size={20}/></button>
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 flex flex-col md:flex-row gap-8 flex-1 print:p-0 print:m-0 print:block">
          {activeTab !== 'home' && (
            <nav className="md:w-64 flex-shrink-0 space-y-4 print:hidden animate-in slide-in-from-left">
              <button onClick={()=>{clearAllReports();
setActiveTab('home');}} className="w-full flex items-center justify-center gap-3 px-5 py-4 text-xs font-black rounded-2xl bg-black text-white shadow-xl hover:bg-gray-800 mb-4 uppercase tracking-widest transition-all active:scale-95"><Home size={18} className="text-orange-500" /> INICIO</button>
              
              {activeTab === 'ventas' && (
                <div className="bg-white rounded-3xl p-5 border border-gray-200 shadow-sm space-y-2">
                  <h3 className="text-[10px] font-black text-gray-500 
uppercase tracking-widest mb-4 flex items-center gap-2 border-b border-gray-100 pb-3"><Receipt size={14} className="text-orange-500"/> Área de Ventas</h3>
                  ...
                  <button onClick={() => {clearAllReports();
setVentasView('requisiciones');}} className={`w-full flex items-center justify-start gap-3 px-5 py-4 text-[11px] font-black rounded-2xl transition-all ${ventasView === 'requisiciones' ?
'bg-black text-white shadow-xl' : 'text-slate-500 hover:bg-slate-100'} uppercase`}><FileText size={16}/> Requisiciones OP</button>
                  <button onClick={() => {clearAllReports();
setVentasView('clientes');}} className={`w-full flex items-center justify-start gap-3 px-5 py-4 text-[11px] font-black rounded-2xl transition-all ${ventasView === 'clientes' ?
'bg-black text-white shadow-xl' : 'text-slate-500 hover:bg-slate-100'} uppercase`}><Users size={16}/> Clientes</button>
                </div>
              )}
           ...
           <h3 className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-4 flex items-center gap-2 border-b border-gray-100 pb-3"><Factory size={14} className="text-orange-500"/> Producción</h3>
                  ...
                  <button onClick={() => {clearAllReports(); setProdView('requisiciones');}} className={`w-full flex items-center justify-start 
gap-3 px-5 py-4 text-[11px] font-black rounded-2xl transition-all ${prodView === 'requisiciones' ?
'bg-black text-white shadow-lg' : 'text-slate-500 hover:bg-gray-50'} uppercase`}><ClipboardList size={16}/> Ingeniería</button>
                  <button onClick={() => {clearAllReports();
setProdView('fases_produccion');}} className={`w-full flex items-center justify-start gap-3 px-5 py-4 text-[11px] font-black rounded-2xl transition-all ${prodView === 'fases_produccion' ?
'bg-black text-white shadow-lg' : 'text-slate-500 hover:bg-gray-50'} uppercase`}><PlayCircle size={16}/> Control Fases</button>
                  <button onClick={() => {clearAllReports();
setProdView('historial');}} className={`w-full flex items-center justify-start gap-3 px-5 py-4 text-[11px] font-black rounded-2xl transition-all ${prodView === 'historial' ?
'bg-black text-white shadow-lg' : 'text-slate-500 hover:bg-gray-50'} uppercase`}><History size={16}/> Historial</button>
                </div>
              )}
...
text-gray-500 uppercase tracking-widest mb-4 flex items-center gap-2 border-b border-gray-100 pb-3"><Package size={14} className="text-orange-500"/> Inventario (Art. 177)</h3>
                  ...
                  <button onClick={() => {clearAllReports(); setInvView('cargo');}} className={`w-full flex items-center justify-start gap-3 px-5 py-4 text-[11px] font-black rounded-2xl transition-all ${invView === 'cargo' ?
'bg-black text-white shadow-xl' : 'text-slate-500 hover:bg-slate-100'} uppercase`}><ArrowDownToLine size={16}/> Cargo</button>
                  <button onClick={() => {clearAllReports();
setInvView('descargo');}} className={`w-full flex items-center justify-start gap-3 px-5 py-4 text-[11px] font-black rounded-2xl transition-all ${invView === 'descargo' ?
'bg-black text-white shadow-xl' : 'text-slate-500 hover:bg-slate-100'} uppercase`}><ArrowUpFromLine size={16}/> Descargo</button>
                  <button onClick={() => {clearAllReports();
setInvView('ajuste');}} className={`w-full flex items-center justify-start gap-3 px-5 py-4 text-[11px] font-black rounded-2xl transition-all ${invView === 'ajuste' ?
'bg-black text-white shadow-xl' : 'text-slate-500 hover:bg-slate-100'} uppercase`}><Settings2 size={16}/> Ajuste</button>
                  <button onClick={() => {clearAllReports();
setInvView('kardex');}} className={`w-full flex items-center justify-start gap-3 px-5 py-4 text-[11px] font-black rounded-2xl transition-all ${invView === 'kardex' ?
'bg-black text-white shadow-xl' : 'text-slate-500 hover:bg-slate-100'} uppercase`}><History size={16}/> Kardex</button>
                  <button onClick={() => {clearAllReports();
setInvView('reporte177');}} className={`w-full flex items-center justify-start gap-3 px-5 py-4 text-[11px] font-black rounded-2xl transition-all ${invView === 'reporte177' ?
'bg-black text-white shadow-xl' : 'text-slate-500 hover:bg-slate-100'} uppercase`}><FileText size={16}/> Reporte Gral.
(Art 177)</button>
                </div>
              )}
            </nav>
          ...
          <main className={`flex-1 min-w-0 pb-12 print:pb-0 print:m-0 print:p-0 print:block print:w-full ${activeTab === 'home' ?
'flex items-center justify-center' : ''}`}>
      ...
      <div className="fixed inset-0 bg-black/85 backdrop-blur-md flex items-center justify-center p-4 z-[9999] print:hidden">
           ...
           {dialog.type === 'confirm' && (<button onClick={() => setDialog(null)} className="flex-1 bg-gray-100 font-black py-4 rounded-2xl uppercase text-[10px] tracking-widest hover:bg-gray-200 transition-colors text-gray-800">CANCELAR</button>)}
                <button onClick={() => { if (dialog.onConfirm) dialog.onConfirm();
setDialog(null); }} className="flex-1 bg-black text-white font-black py-4 rounded-2xl shadow-xl uppercase text-[10px] tracking-widest hover:bg-gray-900 transition-colors">ACEPTAR</button>
              </div>
            </div>
          </div>
        )}
