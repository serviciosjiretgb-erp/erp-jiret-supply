    ...
    if (this.state.hasError) {
      return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-6 text-center">
          ...
          <button onClick={() => window.location.reload()} className="bg-black text-white font-black px-8 py-4 rounded-xl uppercase tracking-widest text-xs shadow-lg">Reiniciar Interfaz</button>
        </div>
      );
    }
...
// ============================================================================
// CONFIGURACIÓN DE FIREBASE (CONSERVANDO TU ESTRUCTURA ORIGINAL)
// ============================================================================
...
export default function App() {
  ...
  // LOGICA PRODUCCIÓN (BLINDADA CONTRA PANTALLA BLANCA)
  const renderRecipeInventoryOptions = () => {
    ...
    return (<><option value="">Seleccione Insumo...</option>{Object.keys(grouped).map(cat => (<optgroup key={cat} label={`📌 ${cat.toUpperCase()}`}>{(grouped[cat] || []).map(i => <option key={i?.id} value={i?.id}>{i?.id} - {i?.desc}</option>)}</optgroup>))}</>);
  };
  ...
  const handleSavePhase = async (e) => {
    ...
    const actionType = e.nativeEvent?.submitter?.name; const isClose = actionType === 'close';
    ...
    if (prodKg > 0 || mermaKg > 0 || (phaseForm?.insumos || []).length > 0) {
        ...
        await batch.commit();
        const newBatch = { id: Date.now().toString(), timestamp: Date.now(), date: phaseForm?.date || getTodayDate(), insumos: phaseForm?.insumos || [], producedKg: prodKg, mermaKg, totalInsumosKg, operator: appUser?.name || 'Operador', techParams: phaseForm };
  ...
  const handleEditBatch = (reqId, phase, batchId) => {
    setDialog({ title: `MODIFICAR LOTE`, text: `El lote volverá al formulario para su edición. ¿Continuar?`, type: 'confirm', onConfirm: async () => {
        ...
        if (bIdx >= 0) { 
            ...
            setPhaseForm({ ...initialPhaseForm, date: batch?.date || getTodayDate(), producedKg: batch?.producedKg || '', mermaKg: batch?.mermaKg || '', insumos: batch?.insumos || [] });
  ...
  const handleReopenPhase = async (reqId, phase) => {
    ...
    let newStatus = req?.status === 'COMPLETADO' ? 'EN PROCESO' : req?.status;
      ...
      <div className="animate-in fade-in space-y-6">
        {invView === 'catalogo' && (
          <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="px-8 py-6 border-b bg-gray-50 flex justify-between items-center"><h2 className="text-xl font-black uppercase flex items-center gap-3"><Box className="text-orange-500" size={24}/> Catálogo</h2></div>
            <div data-html2canvas-ignore="true" className="p-8 bg-gray-50/50 border-b border-gray-200">
               <form onSubmit={handleSaveInvItem} className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                   <div>
                     <label className="text-[10px] font-black text-gray-500 uppercase block mb-1">ID</label>
                     <input type="text" required disabled={!!editingInvId} value={newInvItemForm.id} onChange={e=>setNewInvItemForm({...newInvItemForm, id: e.target.value.toUpperCase()})} className="w-full border-2 border-gray-200 bg-gray-50 focus:bg-white rounded-xl p-3 font-black text-xs uppercase outline-none" />
                   </div>
                   <div className="md:col-span-2">
                     <label className="text-[10px] font-black text-gray-500 uppercase block mb-1">Descripción</label>
                     <input type="text" required value={newInvItemForm.desc} onChange={e=>setNewInvItemForm({...newInvItemForm, desc: e.target.value.toUpperCase()})} className="w-full border-2 border-gray-200 bg-gray-50 focus:bg-white rounded-xl p-3 font-black text-xs uppercase outline-none" />
                   </div>
                   <div className="grid grid-cols-2 gap-2">
                     <div>
                       <label className="text-[10px] font-black text-gray-500 uppercase block mb-1">Costo ($)</label>
                       <input type="number" step="0.01" required value={newInvItemForm.cost} onChange={e=>setNewInvItemForm({...newInvItemForm, cost: e.target.value})} className="w-full border-2 border-gray-200 bg-gray-50 focus:bg-white rounded-xl p-3 font-black text-xs outline-none" />
                     </div>
                     <div>
                       <label className="text-[10px] font-black text-gray-500 uppercase block mb-1">Stock</label>
                       <input type="number" step="0.01" required value={newInvItemForm.stock} onChange={e=>setNewInvItemForm({...newInvItemForm, stock: e.target.value})} className="w-full border-2 border-gray-200 bg-gray-50 focus:bg-white rounded-xl p-3 font-black text-xs outline-none" />
                     </div>
                   </div>
                 </div>
                 <div className="flex justify-end gap-2 pt-4">
                    {editingInvId && <button type="button" onClick={() => {setEditingInvId(null); setNewInvItemForm(initialInvItemForm);}} className="bg-gray-200 text-gray-700 px-6 py-4 rounded-2xl font-black text-[10px] uppercase">CANCELAR</button>}
                    <button type="submit" className="bg-black text-white px-10 py-4 rounded-2xl font-black text-[10px] uppercase">GUARDAR</button>
                 </div>
               </form>
            </div>
            <div className="p-8"><div className="overflow-x-auto"><table className="w-full text-left whitespace-nowrap"><thead className="bg-gray-100 border-b-2 border-gray-200 font-black text-[10px] uppercase"><tr><th>Código</th><th>Descripción</th><th className="text-right">Stock</th><th className="text-center">Acciones</th></tr></thead><tbody className="divide-y divide-gray-100">{filteredInventory.map(inv => (<tr key={inv?.id} className="hover:bg-gray-50"><td className="py-4 px-4 font-black text-orange-600 text-xs">{inv?.id}</td><td className="py-4 px-4 font-black uppercase text-xs">{inv?.desc}</td><td className="py-4 px-4 text-right font-black text-blue-600 text-lg">{formatNum(inv?.stock)} <span className="text-xs text-gray-400">{inv?.unit}</span></td><td className="py-4 px-4 text-center"><div className="flex justify-center gap-2"><button onClick={() => startEditInvItem(inv)} className="p-2 bg-blue-50 text-blue-500 rounded-xl hover:bg-blue-100"><Edit size={16}/></button><button onClick={()=>handleDeleteInvItem(inv?.id)} className="p-2 bg-red-50 text-red-500 rounded-xl hover:bg-red-100"><Trash2 size={16}/></button></div></td></tr>))}</tbody></table></div></div>
          </div>
        )}
      </div>
    );
    ...
    return (
      <div className="space-y-6 animate-in fade-in">
        {ventasView === 'requisiciones' && (
          <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden text-black">
             <div className="px-8 py-6 border-b bg-white flex justify-between items-center"><h2 className="text-xl font-black uppercase flex items-center gap-3 tracking-tighter"><FileText className="text-orange-500" size={24}/> REQUISICIONES</h2><button onClick={()=>{setShowNewReqPanel(!showNewReqPanel);setNewReqForm(initialReqForm);}} className="bg-black text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase shadow-md">{showNewReqPanel ? 'CANCELAR' : 'NUEVA ORDEN'}</button></div>
             {showNewReqPanel && (
                  ...
                  <form onSubmit={handleCreateRequirement} className="bg-white p-10 rounded-3xl border border-gray-100 shadow-sm space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                          ...
                          <select required value={newReqForm.client} onChange={e=>handleReqFormChange('client', e.target.value)} className="w-full bg-white border-2 border-gray-200 rounded-2xl p-4 font-black text-xs">
                            <option value="">Seleccione...</option>{(clients || []).map(c=><option key={c?.rif} value={c?.name}>{c?.name}</option>)}
                          </select>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                          <div><label className="text-[10px] font-bold text-gray-500 uppercase block mb-1">Ancho (cm)</label><input type="number" step="0.1" value={newReqForm.ancho} onChange={e=>handleReqFormChange('ancho', e.target.value)} className="w-full border-2 border-gray-200 rounded-2xl p-5 font-black text-lg text-center" /></div>
                          <div><label className="text-[10px] font-bold text-gray-500 uppercase block mb-1">Fuelle (cm)</label><input type="number" step="0.1" value={newReqForm.fuelles} onChange={e=>handleReqFormChange('fuelles', e.target.value)} className="w-full border-2 border-gray-200 rounded-2xl p-5 font-black text-lg text-center" /></div>
                        </div>
                    </div>
                    <div className="flex justify-between items-center bg-orange-50 p-6 rounded-3xl border-2 border-orange-200 mt-6 shadow-inner"><div><span className="text-[10px] font-black text-orange-800 uppercase tracking-tighter">TOTAL CARGA ESTIMADA</span><span className="text-4xl font-black text-orange-600 block">{newReqForm.requestedKg} KG</span></div><button type="submit" className="bg-orange-500 text-white px-12 py-5 rounded-2xl font-black text-[10px] uppercase shadow-xl">GUARDAR</button></div>
                  </form>
                </div>
             )}
             <div className="p-8 overflow-x-auto"><table className="w-full text-left whitespace-nowrap text-black"><thead className="bg-white border-b-2 border-gray-100 font-black text-[10px] uppercase"><tr><th>OP N°</th><th>Cliente</th><th>KG Est.</th><th>Acciones</th></tr></thead><tbody className="divide-y divide-gray-100">{(requirements || []).map(r=>(<tr key={r?.id} className="hover:bg-gray-50"><td className="py-5 px-4 font-black text-orange-500">#{String(r?.id).replace('OP-','').padStart(5,'0')}</td><td className="py-5 px-4 font-black uppercase text-sm">{r?.client}</td><td className="py-5 px-4 text-right font-black text-lg">{formatNum(r?.requestedKg)}</td><td className="py-5 px-4 text-center"><div className="flex justify-center gap-2"><button onClick={()=>setShowSingleReqReport(r?.id)} className="p-2.5 bg-gray-100 rounded-xl hover:bg-gray-800 hover:text-white"><Printer size={16}/></button><button onClick={()=>startEditReq(r)} className="p-2.5 bg-blue-50 text-blue-500 rounded-xl hover:bg-blue-500 hover:text-white"><Edit size={16}/></button></div></td></tr>))}</tbody></table></div>
          </div>
        )}
      </div>
    );
      ...
      <div className="animate-in fade-in space-y-6">
        {prodView === 'calculadora' && (
          <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden animate-in fade-in print:border-none print:shadow-none print:m-0 print:p-0 print:block print:w-full">
            <div data-html2canvas-ignore="true" className="px-8 py-6 border-b border-gray-200 bg-gray-50 flex justify-between items-center print:hidden">
               <h2 className="text-xl font-black text-black uppercase flex items-center gap-3"><Calculator className="text-orange-500" size={24}/> Simulador</h2>
               <button onClick={() => handleExportPDF('Simulador_Produccion', true)} className="bg-black text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase flex items-center gap-2"><Printer size={16}/> EXPORTAR</button>
            </div>
            <div id="pdf-content" className="grid grid-cols-1 lg:grid-cols-12 gap-0 print:block print:w-full">
               ...
               <div data-html2canvas-ignore="true" className="lg:col-span-4 border-r border-gray-200 bg-gray-50 p-8 print:hidden space-y-8 text-black">
                 <div>
                     <h3 className="text-xs font-black uppercase text-black mb-4 border-b border-gray-200 pb-2">1. Carga (KG)</h3>
                     <input type="number" value={calcInputs?.mezclaTotal} onChange={(e) => handleCalcChange('mezclaTotal', e.target.value)} className="w-full border-2 border-gray-200 rounded-xl p-3 text-sm font-black text-blue-600 outline-none" />
                 </div>
                 <div>
                     <h3 className="text-xs font-black uppercase text-black mb-4 border-b border-gray-200 pb-2">2. Producto</h3>
                     <select value={calcInputs?.tipoProducto} onChange={e=>setCalcInputs({...calcInputs, tipoProducto: e.target.value})} className="w-full border-2 border-gray-200 rounded-lg p-2 text-xs font-black text-blue-600 outline-none">
               ...
               <div className="lg:col-span-8 p-10 bg-white print:w-full print:p-0">
                  <div className="hidden print:block mb-6 text-black"><ReportHeader /><h1 className="text-xl font-black uppercase border-b-2 border-orange-500 pb-1 mt-2">PROYECCIÓN DE PRODUCCIÓN</h1></div>
                  <div className="overflow-x-auto border border-gray-300 print:border-black">
                     <table className="w-full text-left text-[10px] whitespace-nowrap text-black">
                        <thead className="bg-gray-200 print:bg-gray-300 border-b border-gray-400 print:border-black font-black uppercase">
                           <tr><th className="p-2">Concepto</th><th className="p-2 text-center">Cantidad</th><th className="p-2 text-center">U.M.</th><th className="p-2 text-center">Costo U.</th><th className="p-2 text-center">Costo Total</th><th className="p-2">Notas</th></tr>
                        </thead>
                        <tbody className="divide-y divide-gray-200 print:divide-black">
                           <tr className="bg-gray-100 font-bold"><td>Mezcla Procesada</td><td className="text-center">{formatNum(calcTotalMezcla)}</td><td className="text-center">kg</td><td className="text-center">${formatNum(calcCostoPromedio)}</td><td className="text-center">${formatNum(calcCostoMezclaPreparada)}</td><td>Entrada Planta</td></tr>
                           <tr className="bg-green-50 font-black text-green-800"><td>Resultado Estimado</td><td className="text-center">{formatNum(calcProduccionFinalUnidades)}</td><td className="text-center">{simUmFinal}</td><td className="text-center">${formatNum(calcCostoFinalUnidad)}</td><td className="text-center">${formatNum(calcCostoMezclaPreparada)}</td><td>Líquida estimada</td></tr>
                        </tbody>
                     </table>
                  </div>
               </div>
        ...
        {prodView === 'fases_produccion' && (
          <div className="space-y-6">
            {!selectedPhaseReqId ? (
              <div className="p-12 bg-white rounded-3xl border border-gray-200 shadow-sm text-center animate-in fade-in"><div className="bg-black p-5 rounded-full inline-block mb-6 text-orange-500 shadow-lg"><Factory size={40}/></div><h2 className="text-2xl font-black uppercase text-black tracking-tighter mb-2">Control de Producción Activo</h2><div className="mt-12 border-t border-gray-200 pt-8 overflow-x-auto"><table className="w-full text-left text-sm whitespace-nowrap text-black"><thead className="bg-gray-50 border-b border-gray-200 font-black text-[10px] uppercase"><tr><th>OP N°</th><th>Cliente / Producto</th><th>Acción</th></tr></thead><tbody className="divide-y divide-gray-100">{(activeOrders || []).map(r => (<tr key={r?.id} className="group hover:bg-gray-50 transition-colors"><td className="p-4 font-black text-orange-500 text-lg">#{String(r?.id).replace('OP-', '').padStart(5, '0')}</td><td className="p-4 font-black uppercase text-xs">{r?.client}<br/><span className="text-[10px] text-gray-400 font-bold">{r?.desc}</span></td><td className="p-4 text-center"><div className="flex justify-center gap-2"><button onClick={() => setShowWorkOrder(r?.id)} className="bg-white border-2 border-gray-100 px-4 py-3 rounded-xl text-[10px] font-black uppercase flex items-center gap-2" title="Imprimir"><Printer size={16}/> ORDEN TRABAJO</button><button onClick={() => { setSelectedPhaseReqId(r?.id); setActivePhaseTab('extrusion'); setPhaseForm({...initialPhaseForm, date: getTodayDate()}); }} className="bg-black text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase flex items-center gap-2 shadow-md hover:bg-slate-800 transition-all"><PlayCircle size={16}/> ENTRAR A FASES</button></div></td></tr>))}</tbody></table></div></div>
            ) : (() => {
              ...
              return (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in slide-in-from-bottom-8 text-black">
                  <div className="lg:col-span-1 space-y-6">
                    <div className="bg-black rounded-3xl shadow-xl p-8 text-white relative overflow-hidden"><div className="absolute -right-6 -bottom-6 opacity-10"><Factory size={160}/></div><div className="relative z-10"><h2 className="text-4xl font-black uppercase tracking-tighter mb-2">#{String(req.id).replace('OP-', '').padStart(5, '0')}</h2><p className="text-sm font-bold text-gray-300 uppercase leading-relaxed mb-6 border-b border-gray-700 pb-6">{req.client}<br/><span className="text-orange-400 text-lg">{req.desc}</span></p><div className="bg-gray-800 p-5 rounded-2xl border border-gray-700 shadow-inner"><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1 tracking-widest">META A PRODUCIR:</p><p className="text-3xl font-black text-white">{formatNum(req.requestedKg)} KG</p></div></div></div>
                  ...
                  <div className="lg:col-span-2 bg-white rounded-3xl shadow-sm border border-gray-200 p-8 lg:p-10 text-black">
                    ...
                    {cPhase.isClosed ? (
                      <div className="text-center py-10 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200 shadow-inner">
                         ...
                         <button onClick={() => handleReopenPhase(req.id, activePhaseTab)} className="bg-orange-500 text-white px-6 py-2 rounded-xl text-[10px] font-black uppercase shadow-md hover:bg-orange-600 transition-all flex items-center gap-2 mx-auto"><Edit size={14}/> REABRIR FASE PARA EDITAR</button>
                      </div>
                    ) : (
  ...
  const renderWorkOrder = () => {
    ...
    return (
      <div id="pdf-content" className="bg-white p-6 print:p-0 min-h-screen text-black shadow-none border-0 font-black uppercase text-black"><style>{`@media print { @page { size: portrait; margin: 5mm; } }`}</style>
        <div data-html2canvas-ignore="true" className="flex justify-between mb-2 print:hidden"><button onClick={() => setShowWorkOrder(null)} className="bg-gray-100 px-6 py-2 rounded-xl text-xs font-black uppercase hover:bg-gray-200 transition-all">VOLVER</button><button onClick={() => handleExportPDF(`OP_${req.id}`)} className="bg-black text-white px-8 py-2 rounded-xl font-black flex items-center gap-2 text-xs uppercase shadow-lg"><Printer size={16} /> EXPORTAR PDF</button></div>
        ...
        <div className="grid grid-cols-3 text-[9px] font-bold uppercase mb-2 border-b-2 border-black pb-2"><div><p className="mb-1"><span className="w-16 inline-block font-black pr-1">CLIENTE:</span> {req.client}</p><p className="mb-1"><span className="w-16 inline-block font-black pr-1">OP:</span> #{String(req.id).replace('OP-', '').padStart(5, '0')}</p><p><span className="w-16 inline-block font-black pr-1">TIPO:</span> {req.tipoProducto || 'N/A'}</p></div><div><p className="mb-1"><span className="w-20 inline-block font-black pr-1">EMISIÓN OP:</span> {req.fecha}</p><p><span className="w-20 inline-block font-black pr-1 text-orange-600">META (KG):</span> <span className="text-orange-600 font-black">{formatNum(req.requestedKg)} KG</span></p></div><div><p className="mb-1"><span className="w-24 inline-block font-black pr-1">FECHA ENTRADA:</span> __________________</p><p><span className="w-24 inline-block font-black pr-1">FECHA SALIDA:</span> __________________</p></div></div>
  ...
  const renderFiniquito = () => {
    ...
    const totUnid = isTermo ? (selB.reduce((a,b)=>a+parseNum(b?.producedKg),0) || extB.reduce((a,b)=>a+parseNum(b?.producedKg),0)) : selB.reduce((s, b) => s + parseNum(b?.millaresProd || b?.techParams?.millares || 0), 0);
    const unitF = isTermo ? 'KG' : 'Millares';

    return (
      <div id="pdf-content" className="bg-white p-12 print:p-0 min-h-screen text-black shadow-none border-0 font-black uppercase text-black"><style>{`@media print { @page { size: landscape; margin: 5mm; } body { background-color: white !important; } .text-orange-600 { color: #ea580c !important; } }`}</style>
        ...
        <div className="flex justify-between items-end border-b-2 border-black pb-4 mb-6"><div className="flex items-center gap-4"><div className="bg-black text-white p-2 rounded-lg flex items-center print:border print:border-black print:bg-white print:text-black font-black text-2xl font-black">G&B</div><div><h1 className="text-lg font-black uppercase tracking-widest text-black">REPORTE FINAL DE PRODUCCIÓN</h1><p className="text-[10px] font-bold text-gray-500 uppercase">SERVICIOS JIRET G&B, C.A.</p></div></div><div className="text-right"><p className="text-[10px] font-bold text-gray-500 uppercase">FECHA EMISIÓN: <span className="text-black font-black">{getTodayDate()}</span></p><p className="text-sm font-black text-orange-600 uppercase mt-1">OP N° {String(req?.id).replace('OP-','').padStart(5,'0')}</p></div></div>
        <div className="grid grid-cols-4 gap-4 bg-gray-50 p-4 rounded-xl mb-6 text-[10px] font-bold uppercase border border-gray-200"><div><span className="text-gray-500 block mb-1">CLIENTE:</span> {req?.client}</div><div className="col-span-2"><span className="text-gray-500 block mb-1">PRODUCTO:</span> {req?.desc}</div><div className="text-right"><span className="text-gray-500 block mb-1">META SOLICITADA:</span> <span className="text-orange-600 font-black text-sm">{formatNum(req?.requestedKg)} KG</span></div></div>
    ...
    <ErrorBoundary>
      <div className="min-h-screen bg-gray-100 text-gray-900 font-sans flex flex-col print:bg-white print:block print:w-full overflow-x-hidden print:overflow-visible text-black font-black">
        <header className="bg-black border-b-4 border-orange-500 sticky top-0 z-50 text-white shadow-md print:hidden">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><div className="flex justify-between h-20 items-center"><div className="flex items-center gap-4 cursor-pointer transition-transform hover:scale-105" onClick={()=>{clearAllReports(); setActiveTab('home');}}><div className="flex items-center bg-white rounded-2xl px-3 py-1 shadow-inner"><span className="text-black font-black text-3xl leading-none">G</span><span className="text-orange-500 font-black text-2xl mx-0.5">&amp;</span><span className="text-black font-black text-3xl leading-none">B</span></div><div className="hidden sm:block border-l-2 border-gray-800 pl-4 uppercase font-black text-lg">Supply ERP</div></div><div className="flex items-center gap-5"><div className="flex items-center gap-3 bg-gray-800 px-5 py-2 rounded-2xl border border-gray-700 shadow-inner"><ShieldCheck size={18} className="text-orange-500" /><span className="font-black text-white text-[10px] uppercase leading-none">{appUser?.name}</span></div><button onClick={() => setAppUser(null)} className="text-gray-400 hover:text-white transition-all bg-gray-800 hover:bg-red-600 p-2.5 rounded-2xl border border-gray-700 shadow-lg"><LogOut size={20}/></button></div></div></div>
        </header>
            ...
            <nav className="md:w-64 flex-shrink-0 space-y-4 print:hidden animate-in slide-in-from-left">
              ...
              {activeTab === 'ventas' && (
                <div className="bg-white rounded-3xl p-5 border border-gray-200 shadow-sm space-y-2">
                  <h3 className="text-[10px] font-black text-gray-500 uppercase mb-4 border-b pb-3 tracking-widest">Ventas</h3>
              ...
              {activeTab === 'produccion' && (
                <div className="bg-white rounded-3xl p-5 border border-gray-200 shadow-sm space-y-2">
                  <h3 className="text-[10px] font-black text-gray-500 uppercase mb-4 border-b pb-3 tracking-widest">Producción</h3>
              ...
              {activeTab === 'inventario' && (
                <div className="bg-white rounded-3xl p-5 border border-gray-200 shadow-sm space-y-2">
                  <h3 className="text-[10px] font-black text-gray-500 uppercase mb-4 border-b pb-3 tracking-widest">Inventario</h3>
