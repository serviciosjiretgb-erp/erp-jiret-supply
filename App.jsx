...
export default function App() {
  ...
  const initialCalcInputs = useMemo(() => ({ 
    ingredientes: [{ id: Date.now() + 1, nombre: 'MP-0240', pct: 80, costo: 0.96 }, { id: Date.now() + 2, nombre: 'MP-RECICLADO', pct: 20, costo: 1.00 }], cantidadSolicitada: '', mermaGlobalPorc: 5, tipoProducto: 'BOLSAS', ancho: '', fuelles: '', largo: '', micras: '' 
  }), []);
  const [calcInputs, setCalcInputs] = useState(initialCalcInputs);

  // Formularios Inventario y Costos Operativos
  const initialInvItemForm = useMemo(() => ({ id: '', desc: '', category: 'Materia Prima', unit: 'kg', cost: '', stock: '' }), []);
  const [newInvItemForm, setNewInvItemForm] = useState(initialInvItemForm);
  const [editingInvId, setEditingInvId] = useState(null);
  
  const initialMovementForm = useMemo(() => ({ date: getTodayDate(), itemId: '', type: 'ENTRADA', qty: '', cost: '', reference: '', notes: '', opAsignada: '' }), []);
  const [newMovementForm, setNewMovementForm] = useState(initialMovementForm);
  
  const initialOpCostForm = useMemo(() => ({ date: getTodayDate(), category: 'Nómina', description: '', amount: '' }), []);
...
const renderHome = () => {
  ...
  const renderProductionModule = () => {
    ...
    if (prodView === 'calculadora') {
      return (
          <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden animate-in fade-in print:border-none print:shadow-none print:m-0 print:p-0 print:block w-full">
            <div data-html2canvas-ignore="true" className="px-8 py-6 border-b border-gray-200 bg-gray-50 flex justify-between items-center no-pdf">
      ...
      return (
          <div className="space-y-6 w-full">
            {!selectedPhaseReqId ? (
              ...
              return (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in slide-in-from-bottom-8 w-full">
                      ...
                      <form onSubmit={handleSavePhase} className="space-y-8">
                        ...
                        {activePhaseTab === 'extrusion' && (
                           <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200 space-y-4">
                              <h4 className="text-[10px] font-black text-gray-600 uppercase flex items-center gap-2 border-b border-gray-200 pb-2"><Settings2 size={16}/> Parámetros Extrusión</h4>
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <div><label className="text-[9px] font-bold text-gray-500 uppercase">Operador</label><input type="text" value={phaseForm?.operadorExt||''} onChange={e=>setPhaseForm({...phaseForm, operadorExt: e.target.value.toUpperCase()})} className="w-full border-2 border-gray-200 p-2 text-xs font-bold rounded-lg uppercase" /></div>
                                <div><label className="text-[9px] font-bold text-gray-500 uppercase">Tratado (L/P)</label><input type="text" value={phaseForm?.tratado||''} onChange={e=>setPhaseForm({...phaseForm, tratado: e.target.value.toUpperCase()})} className="w-full border-2 border-gray-200 p-2 text-xs font-bold rounded-lg uppercase" /></div>
                                <div><label className="text-[9px] font-bold text-gray-500 uppercase">Motor (Amp/Hz)</label><input type="text" value={phaseForm?.motorExt||''} onChange={e=>setPhaseForm({...phaseForm, motorExt: e.target.value.toUpperCase()})} className="w-full border-2 border-gray-200 p-2 text-xs font-bold rounded-lg uppercase" /></div>
                                <div><label className="text-[9px] font-bold text-gray-500 uppercase">Ventilador (Hz)</label><input type="text" value={phaseForm?.ventilador||''} onChange={e=>setPhaseForm({...phaseForm, ventilador: e.target.value.toUpperCase()})} className="w-full border-2 border-gray-200 p-2 text-xs font-bold rounded-lg uppercase" /></div>
                              </div>
                           </div>
                        )}
                        ...
                        {activePhaseTab === 'impresion' && (
                           <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200 space-y-4">
                              <h4 className="text-[10px] font-black text-gray-600 uppercase flex items-center gap-2 border-b border-gray-200 pb-2"><Settings2 size={16}/> Parámetros Impresión</h4>
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <div><label className="text-[9px] font-bold text-gray-500 uppercase">Operador</label><input type="text" value={phaseForm?.operadorImp||''} onChange={e=>setPhaseForm({...phaseForm, operadorImp: e.target.value.toUpperCase()})} className="w-full border-2 border-gray-200 p-2 text-xs font-bold rounded-lg uppercase" /></div>
                                <div><label className="text-[9px] font-bold text-gray-500 uppercase">Colores</label><input type="number" value={phaseForm?.cantColores||''} onChange={e=>setPhaseForm({...phaseForm, cantColores: e.target.value})} className="w-full border-2 border-gray-200 p-2 text-xs font-bold rounded-lg" /></div>
                                <div><label className="text-[9px] font-bold text-gray-500 uppercase">Solvente (Lts)</label><input type="number" step="0.1" value={phaseForm?.solvente||''} onChange={e=>setPhaseForm({...phaseForm, solvente: e.target.value})} className="w-full border-2 border-gray-200 p-2 text-xs font-bold rounded-lg" /></div>
                                <div><label className="text-[9px] font-bold text-gray-500 uppercase">Relación Piñón</label><input type="text" value={phaseForm?.relacionImp||''} onChange={e=>setPhaseForm({...phaseForm, relacionImp: e.target.value})} className="w-full border-2 border-gray-200 p-2 text-xs font-bold rounded-lg" /></div>
                              </div>
                           </div>
                        )}
                        ...
                        {activePhaseTab === 'sellado' && (
                           <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200 space-y-4">
                              <h4 className="text-[10px] font-black text-gray-600 uppercase flex items-center gap-2 border-b border-gray-200 pb-2"><Settings2 size={16}/> Parámetros Sellado</h4>
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <div><label className="text-[9px] font-bold text-gray-500 uppercase">Operador</label><input type="text" value={phaseForm?.operadorSel||''} onChange={e=>setPhaseForm({...phaseForm, operadorSel: e.target.value.toUpperCase()})} className="w-full border-2 border-gray-200 p-2 text-xs font-bold rounded-lg uppercase" /></div>
                                <div><label className="text-[9px] font-bold text-gray-500 uppercase">Impresa</label><select value={phaseForm?.impresa||'NO'} onChange={e=>setPhaseForm({...phaseForm, impresa: e.target.value})} className="w-full border-2 border-gray-200 p-2 text-xs font-bold rounded-lg"><option value="SI">SI</option><option value="NO">NO</option></select></div>
                                <div><label className="text-[9px] font-bold text-gray-500 uppercase">Tipo Sello</label><select value={phaseForm?.tipoSello||'Sello FC'} onChange={e=>setPhaseForm({...phaseForm, tipoSello: e.target.value})} className="w-full border-2 border-gray-200 p-2 text-xs font-bold rounded-lg"><option value="Sello FC">Sello FC</option><option value="Sello Lateral">Sello Lateral</option></select></div>
                                <div><label className="text-[9px] font-bold text-gray-500 uppercase">Millares Prod.</label><input type="number" step="0.1" value={phaseForm?.millaresProd||''} onChange={e=>setPhaseForm({...phaseForm, millaresProd: e.target.value})} className="w-full border-2 border-gray-200 p-2 text-xs font-bold rounded-lg" /></div>
                              </div>
                           </div>
                        )}
