    ...
    const save = async()=>{
      ...
      try {
        ...
        if((form.tipo==='Transferencia'||form.tipo==='Traslado de Fondo')&&cuentaDest) {
          batch.update(getDocRef('banco_cuentas',cuentaDest.id),{saldo:Number(cuentaDest.saldo)+mNat});
...
<div className="grid grid-cols-12 gap-3 bg-slate-50 p-4 rounded-xl border border-slate-100">
  ...
  <div className="col-span-12 md:col-span-4 mt-2">
    {form.tipo==='Traslado de Fondo'?(
      <CuentaSelector value={form.cuentaDestinoId} onChange={v=>setForm({...form,cuentaDestinoId:v})} label="Cuenta Bancaria Destino" excluirId={form.cuentaId}/>
    ):(
      <BFG label="Cuenta Contable (Contrapartida)">
    ...
    <div className="col-span-12 md:col-span-4 mt-2">
    <BFG label="Monto Divisas (USD)">
      <div className="relative">
        <span className="absolute left-3 top-2.5 text-slate-400 font-bold text-xs">$</span>
        <input type="number" step="0.01" min="0.01" className={`${inp} pl-8 font-black text-emerald-600 bg-white`} value={form.montoUSD} onChange={e=>{
          const v=e.target.value; const usdNum=Number(v)||0;
          const nativo=bs?(usdNum*(Number(form.tasa)||tasaActiva)):usdNum;
          setForm({...form,montoUSD:v,montoNativo:String(nativo)});
        }} placeholder="0.00"/>
      </div>
    </BFG>
  </div>
      ...
      <BFG label="Tasa BCV">
        <div className="relative">
          <input type="number" step="0.01" className={`${inp} bg-white`} value={form.tasa} onChange={e=>{
            const v=e.target.value; const usdNum=Number(form.montoUSD)||0;
            const nativo=bs?(usdNum*(Number(v)||tasaActiva)):usdNum;
            setForm({...form,tasa:v,montoNativo:String(nativo)});
          }}/>
    ...
    <div className="flex-1">
      <BFG label="Equiv. (Bs)">
        <div className="w-full bg-slate-900 text-white rounded-lg p-2 flex items-center justify-center h-[38px] shadow-inner">
           <span className="font-mono font-bold text-sm">Bs. {bancoFmt((form.montoUSD || 0) * (form.tasa || 1))}</span>
        </div>
      </BFG>
    </div>
  </div>
